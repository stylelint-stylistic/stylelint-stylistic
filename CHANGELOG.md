...
- False negatives are fixed — a violation the plugin used to pass over in silence, whether `--fix` was asked for or not, is now reported:
	...
	- The `media-feature-colon-space-*`, `media-feature-range-operator-space-*` and `media-query-list-comma-*` rules now read a media query whose parenthesis stands behind something that names no call (see [#230](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/230)). A name is the whole run of name characters in front of the parenthesis, and it is a name only where that run opens on a letter or an underscore, hyphens in front of it aside, and closes on a letter, an underscore or a digit. So nothing is named in front of the parentheses of `@media -(min-width:1px)`, `@media 2-(width>=1px)`, `@media 2(a,b)` or `@media 2and(a,b)`; all four used to be read as a call's arguments, which these eight rules pass over, and every colon, range operator and comma inside them went unreported. The reading is narrower than an identifier of CSS, so a real call spelled in one of the four shapes [#252](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/252) carries loses its arguments to these rules the other way about: `@media a-(max-width:600px)` and `@media fo\6f(max-width:2px)` are read as a query now, and a fix written there lands inside a call.
