...
#### Passing over `@charset`

The [`at-rule-name-case`](https://stylelint-stylistic.github.io/rules/at-rule-name-case), [`at-rule-name-space-after`](https://stylelint-stylistic.github.io/rules/at-rule-name-space-after), [`at-rule-name-newline-after`](https://stylelint-stylistic.github.io/rules/at-rule-name-newline-after), [`at-rule-semicolon-space-before`](https://stylelint-stylistic.github.io/rules/at-rule-semicolon-space-before) and [`string-quotes`](https://stylelint-stylistic.github.io/rules/string-quotes) rules no longer warn about a `@charset` or write into it, in any spelling: the encoding declaration is a byte sequence the browser reads before parsing rather than an at-rule, and every other spelling of it is ignored. Stylelint's [`at-charset-rule-no-invalid`](https://stylelint.io/user-guide/rules/at-charset-rule-no-invalid) rule judges the spelling instead, so a CSS file holding a `@charset` now gets one warning asking you to enable that rule where your configuration leaves it off. The [`unicode-bom`](https://stylelint-stylistic.github.io/rules/unicode-bom) rule under `"always"` now warns about the option itself over a CSS file whose first node is a `@charset`, since the mark would keep the declaration from working; set `"never"` or leave the rule unset for such files.
...
- The [`at-rule-name-newline-after`](https://stylelint-stylistic.github.io/rules/at-rule-name-newline-after) rule is now autofixable (see [#696](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/696)). Expect `--fix` to write the newlines the rule used to only ask for. Where your configuration also lists `at-rule-name-space-after` under `always`, the two ask for different things in the same place, and the plugin now refuses the pair as [contradicting settings](https://stylelint-stylistic.github.io/user-guide/contradicting-settings). A `@charset` is reported but left as written, since breaking its head can leave the stylesheet declaring no encoding.
...
- The [`block-closing-brace-space-after`](https://stylelint-stylistic.github.io/rules/block-closing-brace-space-after) rule is now autofixable (see [#698](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/698)). Expect `--fix` to write the whitespace the rule used to only ask for.
- The [`media-query-list-comma-newline-before`](https://stylelint-stylistic.github.io/rules/media-query-list-comma-newline-before) rule is now autofixable (see [#699](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/699)). Expect `--fix` to write the newlines the rule used to only ask for. Where your configuration also lists `media-query-list-comma-space-before` asking for something else of that same run — `media-query-list-comma-newline-before: "always-multi-line"` beside `media-query-list-comma-space-before: "never"`, for one — the plugin now refuses the pair as [contradicting settings](https://stylelint-stylistic.github.io/user-guide/contradicting-settings).
- The [`unicode-bom`](https://stylelint-stylistic.github.io/rules/unicode-bom) rule is now autofixable (see [#700](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/700)). Expect `--fix` to write or take off the byte order mark the rule used to only ask for.
- The [`value-list-comma-newline-before`](https://stylelint-stylistic.github.io/rules/value-list-comma-newline-before) rule is now autofixable (see [#701](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/701)). Expect `--fix` to write the newlines the rule used to only ask for. Where your configuration also lists `value-list-comma-space-before` asking for something else of that same run — `value-list-comma-newline-before: "always-multi-line"` beside `value-list-comma-space-before: "never"`, for one — the plugin now refuses the pair as [contradicting settings](https://stylelint-stylistic.github.io/user-guide/contradicting-settings).
