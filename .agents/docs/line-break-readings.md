...
A name that reads one spelling of a break alone, as `CRLF` does, says which question it is allowed to answer, since a reader picking a name cannot tell a narrow one from a wide one otherwise. Adding a narrow name without that sentence is how #245 and #247 came to be written.
