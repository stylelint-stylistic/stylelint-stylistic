...
- `lib/preprocessor/` — the machinery only a syntax's adapter reads: the `//`-comment probe and the reader's and writer's questions over it, the selector copy triad, `requiresTrailingSemicolon`. Nothing under `lib/rules/` or `lib/utils/` imports out of it; what still has a consumer in the core — the span scanners, the `has*Interpolation` family — stays in `lib/utils/` until the SCSS phase resolves those consumers.
- `lib/rules/<rule-name>/` — `index.ts` (rule), `index.test.ts`, `README.md` (user docs).
- `lib/utils/<utilName>/index.ts` — one util per directory. Many rules are thin wrappers over shared checkers: `whitespaceChecker` (the `always`/`never`/`always-single-line`/… engine behind most `*-space-*` and `*-newline-*` rules), plus the `*CommaSpaceChecker`, `*ColonSpaceChecker`, `isStandardSyntax*`, and `has*Interpolation` families. Look for an existing util before writing new traversal logic — and note that a rule reaches the syntax-aware ones (the `isStandardSyntax*` and `has*Interpolation` families, the inline-comment machinery, the `get`/`set` pairs) only through the `Syntax` contract of `lib/syntaxes/index.ts`, never by importing them. A util puts its helpers above what it exports, and a type is exported where it is declared.
...
- `let` binds; `const` names a constant. A constant is a fixed value the module names once, written in `SCREAM_CASE` at the top level, and the two spellings imply each other: nothing in `SCREAM_CASE` is `let`, and nothing spelled otherwise is `const`. A binding that merely happens never to be reassigned is no constant — the `WeakMap` cache of [readsInlineComments](lib/utils/readsInlineComments/index.ts), the plugin array [lib/index.ts](lib/index.ts) hangs a getter on, and the `testRule` a test file binds all stay `let`. So do the four names a rule module opens with, `shortName`, `ruleName`, `messages` and `meta`, fixed as every one of them is: Stylelint and the testing library read three of them under those very names, so none of the four can be shouted without the block falling apart.
