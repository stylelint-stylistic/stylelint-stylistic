...
- `lib/preprocessor/` — the machinery only a syntax's adapter reads: the `//`-comment probe and the reader's and writer's questions over it, and the selector copy triad. Nothing under `lib/rules/` or `lib/utils/` imports out of it; what still has a consumer in the core — the span scanners, the `has*Interpolation` family — stays in `lib/utils/` until the SCSS phase resolves those consumers.
