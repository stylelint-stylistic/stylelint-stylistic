...
- `lib/preprocessor/` — what the two preprocessor namespaces share and the core does not read: the `//`-comment probe and the reader's and writer's questions over it, the pair of copies `postcss-scss` keeps (`printedText`) with the selector copy triad over it, the guards composing the constructs both languages spell over the core's (`guards/`), and the interpolation spellings — `regexps.ts`, `hasInterpolation`, the scanner `findInterpolationSpans`. Nothing under `lib/rules/` or `lib/utils/` imports out of it, which `lib/syntaxes/index.test.ts` enforces; the core keeps the shape of a span and `hasTplInterpolation`.
- `lib/rules/<rule-name>/` — `index.ts` (rule), `index.test.ts`, `README.md` (user docs).
- `lib/utils/<utilName>/index.ts` — one util per directory. Many rules are thin wrappers over shared checkers: `whitespaceChecker` (the `always`/`never`/`always-single-line`/… engine behind most `*-space-*` and `*-newline-*` rules), plus the `*CommaSpaceChecker`, `*ColonSpaceChecker` and `isStandardSyntax*` families. Look for an existing util before writing new traversal logic — and note that a rule reaches the syntax-aware ones (the `isStandardSyntax*` family, the inline-comment machinery, the `get`/`set` pairs) only through the `Syntax` contract of `lib/syntaxes/index.ts`, never by importing them. A util puts its helpers above what it exports, and a type is exported where it is declared.
- `lib/regexps.ts` — every regular expression the plugin reads a stylesheet with, each one a named constant under a line saying what it matches. Nothing else in `lib/` spells a pattern inline, so a pattern wanted in a second place is already there under a name, and whatever had to be worked out about it is written once. A namespace keeps the expressions only its adapter reads in a `regexps.ts` of its own, under the same rules — `lib/syntaxes/less/regexps.ts` is the first — and `lib/preprocessor/regexps.ts` holds the spellings both preprocessor namespaces read.
