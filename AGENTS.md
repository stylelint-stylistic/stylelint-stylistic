...
Every task goes through the [`Makefile`](Makefile), which is the list of tasks: read it, or run `make help` for every target with its description and its flags. Nothing is repeated here, so no list can fall behind the one that runs. It puts `node_modules/.bin` on `PATH`. The only npm scripts are `help`, which calls `make help`, and `prepare`, which is [scripts/prepare.ts](scripts/prepare.ts): it points Git at [.githooks](.githooks) and links `.claude/skills`, each where there is a repository and a `.claude/` to do it in. **It does not build, and the build must not be put back into it.** `exports` names `dist/index.js`, so a dependency named by a Git reference either stops at the consumer's gate for build scripts or installs a package that throws `ERR_MODULE_NOT_FOUND` on import — both measured, stated in [README.md](README.md), and the accepted price. Measured on 2026-09-05: `prepack` is not an answer, since neither npm 11.19 nor npm 12.0.2 runs it for a Git dependency, and the package it packs is broken in the same way with nothing said at install; while a `prepare` script makes whoever names a Git reference pass a gate — pnpm ≥ 10 refuses outright (`ERR_PNPM_GIT_DEP_PREPARE_NOT_ALLOWED`, wanting `allowBuilds` in `pnpm-workspace.yaml`), npm 12 refuses a Git reference at all by default (`EALLOWGIT`, wanting `allow-git` in `.npmrc`), and neither reads that permission out of the consumer's `package.json`, so it cannot travel in a reproduction demo. That gate hangs on the script's mere presence and not on what it does — a Git reference to a copy of this package with no scripts at all installs with no gate, measured both ways — so removing the build does not lift it. Whoever installs a published version passes nothing: `prepare` is not run for a dependency that comes from the registry, which is why nobody has ever had to allow `stylelint` itself, and it ships a `prepare` of its own. What the build was for — showing a reporter a fix before it ships — is served by publishing instead, since a fix here goes out as a patch version within the day.
