...
/** How the three letters of the name are spelled, and what stands in front of them: the five spellings CSS reads as `url`, the six runs that leave an ordinary call whose arguments are code, the four things that close a state a scan may be in — a quotation mark, the delimiter of a block comment, the break of an end-of-line one, and the parenthesis an earlier address closes on — which are no name and which a reader keeping a name's start has to say so about, and a backslash in front of each of the four newlines the grammar reads, which spells nothing and is a delimiter of its own, so that the three letters behind it are the name they spell and the same delimiter inside them cuts them in two (#566). */
