 ...
 *
 * Written for spec 1789403758, the offspring of [#653](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/653): `property-case`, `at-rule-name-case`, `media-feature-name-case`, `selector-pseudo-class-case`, `selector-pseudo-element-case` and `color-hex-case` recased a name with `toUpperCase` and `toLowerCase`, so `fß` became `FSS` and `Fİ` became `fi̇`: another name, since a name is ASCII case-insensitive and every other code point is part of it as it stands. The code points are those of `non-ascii-unit`, crossed with the letter `f`, which is a hex digit too, so a hash holding it is read by the colour rule. Each stands alone as the name, and at the head, in the middle and at the tail of one beside the letter.
 *
 * The ASCII names are the control of the recase: the branch recases them exactly as the base did, so their rows must not move. A hash opening on the code point is a control of another kind: no hex digit opens it, so the colour rule reads no colour there on either side, except that the `i` flag of `HEX_COLOR` folds the Kelvin sign into `k` and the long s into `s`, so a hash opening on either of those two is read as a colour and moves like the rest.
 */
