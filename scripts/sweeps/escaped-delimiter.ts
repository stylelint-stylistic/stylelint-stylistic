 ...
 *
 * The copy used to hide strings and comments from the search and not escapes, so `a\,b` was a list of two and a rule wrote whitespace beside the backslash. The delimiter stands behind one to three backslashes, since the run's parity decides whether it is escaped; a hexadecimal spelling of the comma is written closed by a space, by a line break, which the copy keeps, and by nothing, and behind the first and the last a plain comma; an escaped space in front of a comma, a colon, a range operator or a bang is the one write the bug made, and the one the `-before` rules now read over the copy, with an escaped tab, a hexadecimal escape closed by a space in front of each delimiter, one closed by a break in front of a comma, and a backslash in front of a break and each delimiter, where the backslash is a delimiter itself and a write behind it is read as its escape, beside it; a hexadecimal letter closing a call's name puts a comma inside a call the search opens only behind a letter. The run behind the delimiter spells every whitespace the options distinguish, and a break in front of the backslashes makes the node multi-line, so the lineness options are reached.
 *
