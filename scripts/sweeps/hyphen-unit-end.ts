 ...
 *
 * `unit-case` read the hyphen as a code point of the unit's identifier and Less reads it as the sign of the operand behind it. The hyphen is spelled bare and in runs of two, three and four, since Less reads the first as its operator and the second as the operand's sign while a third leaves a keyword, behind none to three backslashes, since the run's parity decides whether it is an operator or a character of an escape, and with whitespace on either side, where the word parts before the rule ever sees it. The head carries the hyphen at the front and inside, and each thing the hyphen may weld on stands behind the dimension.
 *
