/**
 * The address of an `@import`, and of the at-rules a namespace reads one behind — Sass's `@use` and `@forward`, Less's `@plugin` —, under every spelling of the at-rule's name, of what stands between the name and the address, and of where the at-rule itself stands.
 *
...
/** Where the at-rule stands: as a statement, with a call of its own behind it, inside each thing that is text rather than code, and in the two places a statement cannot open in at all, which the walk reads all the same. The string is quoted with the mark its content does not hold, so that the at-rule stands inside it whichever mark the address is written with. Every combination is put to three parsers and about a seventh of the rows is a text one of them refuses; such a row is stable and moves for nobody. */
