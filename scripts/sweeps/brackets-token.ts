 ...
 *
 * A break written into such a token makes the parentheses code, and a `[` inside, or a `{` in a custom property's value, is then a group nothing closes, so the output stops parsing; no oracle carries a bracket inside parentheses. The contents whose group is closed, or which the parser passes over, are the control set, expected to move nowhere. The places without a name in front of the parentheses, where the comma is a value list's and not a call's, were added with a multi-line list among them since the list's lineness is read over the whole declaration. The parentheses of a media feature, where the comma is a media query list's and a `{` opens a group whatever the at-rule, were added with the brace contents beside a comma, and a multi-line list among them likewise.
 */
