 ...
 *
 * Written for 1789556602: a break written into such a token makes the parentheses code, and a `[` inside, or a `{` in a custom property's value, is then a group nothing closes, so the output stops parsing; no oracle carries a bracket inside parentheses. The contents whose group is closed, or which the parser passes over, are the control set, expected to move nowhere. The places without a name in front of the parentheses, where the comma is a value list's and not a call's, were added for 1789637408, with a multi-line list among them since the list's lineness is read over the whole declaration.
 */
...
/** Where the call stands: a value, an ordinary and a custom property's, glued behind a sign as an address is, nested in another call, in front of a bang, and in an at-rule's params and a selector, which no rule here writes into but the control of the places; then the same parentheses without a name, in a value, a custom property's, glued behind a sign, in front of a bang, and in a value list made multi-line by a break outside them. */
...
/** The three rules writing a break inside a call's parentheses and the two writing one beside a value list's comma, and the space twins and the comma space rules as the control, which write no break. */
