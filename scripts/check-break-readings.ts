 ...
 *
 * [AGENTS.md](../AGENTS.md) wants every stylesheet-reading pattern under a name in [lib/regexps.ts](../lib/regexps.ts); a `===` against a break, an `includes`, a `style-search` target and a template-literal pattern slip past that, and `oxlint` has no `no-restricted-syntax`. Matching the shapes of a reading cannot work (a first draft doing that missed a real one), so every line spelling a break is a finding until a list classifies it.
 *
...
const DEBT: Record<string, string[]> = {
	"lib/rules/block-closing-brace-empty-line-before/index.ts": [`if (text[index - 1] === \`\\r\`) index -= 1`],
	"lib/rules/block-closing-brace-newline-before/index.ts": [`if (text[index - 1] === \`\\r\`) index -= 1`],
