 ...
 *
 * This is the mirror image of `comments.mjs`, and the half of the question that oracle cannot ask. That one counts the comment openings a file holds, so it catches a comment a fixer deleted; the opposite failure is code a fixer swallowed *into* a comment that survives, and to a count of comments nothing has happened at all. Nothing else sees it either: the output parses, since a commented-out declaration leaves a stylesheet every parser is happy with, and the run after it is stable. #248 is that shape, and `converge.mjs` reports no row for either of its rules.
 *
 ...
 * @param {string} code - The stylesheet to count in.
 * @param {string} syntaxName - The syntax it is written in.
 * @returns {string | null} The tally, as a key two stylesheets can be compared by, or null where the text does not parse.
 */
function tally (code, syntaxName) {
...
async function probe (run) {
	...
	// Output that no longer parses is `converge.mjs`'s finding rather than this one's, and reporting it here would say the same thing twice
