...
/** Where the objects of the fabricated side go. `git mktree` cannot be told not to write the tree it builds, and a tree no commit points at has no place in the database the worktrees share, so it is written here and read beside the real one through `GIT_ALTERNATE_OBJECT_DIRECTORIES`. What `treeOf` writes for the working tree goes on going where it goes, as every run of an oracle or a sweep writes one too. It stands under `tmp/`, as the scratch index of `cache.ts` does. */
let objects: string

/** What `GIT_ALTERNATE_OBJECT_DIRECTORIES` stood at before, nothing unless a caller had it pointed somewhere already. */
let alternatesBefore: string | undefined

/**
 * Runs Git in the repository, writing into the file's own database, and hands back what it printed. Neither subcommand put to it reads an object, and under these variables the real database is neither the primary one nor among the alternates.
...
function treeOfEntries (entries: string[]): string {
	// The blob the entries name was never written, `hash-object` having been asked for its hash alone, and a tree naming a missing object is refused without this
	return git([`mktree`, `--missing`], `${entries.join(`\n`)}\n`)
}

/**
 * Fabricates a side whose every path the key hashes holds something else than the working tree does, so that a case asking where an input is taken from cannot pass by the two standing the same. No revision of this repository would do: the newest one a shallow clone reaches is `HEAD`, whose files are the ones on disk.
...
describe(`what an oracle result is kept under`, () => {
	...
	it(`is all the comparison keys a result on, since that file names no hash of its own`, () => {
		// The comparison reads `argv`, runs every oracle and writes the report as it loads, so no case can import it; it is read as text instead, as `scripts/sweeps/key.test.ts` holds the same rule about the runner of the sweeps
