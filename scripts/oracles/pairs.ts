 ...
 *
 * Stylelint runs rules in configuration order, once, so a rule formatting text another rule writes runs before that text exists or after it.
 *
 * Kinds of row:
 *
 * - `cycle` — one order never comes to rest within the passes given; the row carries the cycle's files with their warnings.
...
/** Short, dirty shapes, read as CSS alone: a pair races over the shape of the text, not its syntax. Only `free-semicolon`, `trailing-run`, `root-no-value` and `root-custom-run` end on something other than a line break, the one way to the fix of `no-missing-end-of-source-newline`, so a class of those races went unseen until `free-semicolon`; `trailing-run` guards a fix rather than reporting one. `root-no-value` is a top-level declaration, where the run behind the colon and the tail of the file are one text; `root-custom-run` and `root-custom-tail` are the custom-property half of that, the tail inside `decl.value`, two fixtures because only the unclosed one wakes `no-missing-end-of-source-newline`. `ratio-number` pairs `aspect-ratio-notation` with the `value-slash-space-*` rules over one solidus; `ratio-feature` is the same in a media query, with the `media-feature-slash-space-*` rules. `grid-shorthand` puts the rows `named-grid-areas-alignment` rewrites beside a solidus the `value-slash-space-*` rules rewrite; `grid-table` adds line names of two widths and a doubled run in front of the solidus, for `no-multiple-whitespaces`. `block-free-semicolon`, `block-free-semicolon-lines` and `comment-free-semicolon` put the stray semicolon inside a block, in front of its closing brace, where `free-semicolon` at the root cannot reach it: `block-closing-brace-space-before` writes the run between that semicolon and the brace while `no-extra-semicolons` or `declaration-block-trailing-semicolon` takes the semicolon away, and the two orders leave two files. `semicolon-comment-lines` puts a comment behind the block's last semicolon, whose run `declaration-block-semicolon-space-after` writes and `declaration-block-trailing-semicolon` takes the semicolon out of. `nested-free-semicolon-lines`, `bodiless-lines` and `no-semicolon-lines` are the controls of that run, a free semicolon behind a nested rule's brace, a bodiless at-rule and a declaration closed by the brace alone. `no-value-run` and `custom-no-value-run` put a wordless declaration in front of the closing brace, where the run behind the colon is the run in front of the brace: the `declaration-colon-*-after` rules write it as the head run and the `block-closing-brace-*-before` rules as the block's, and a custom property keeps it in its value; two spaces, which no option accepts, wake every rule of both families. */
const CORPUS: [string, string][] = [
	...
	[`block-free-semicolon`, `a {color: pink; ;\n}\n`],
	[`block-free-semicolon-lines`, `a {\n\tcolor: pink;\n\n;\n\n}\n`],
	[`comment-free-semicolon`, `a {/*c*/\n;}\n`],
	[`semicolon-comment-lines`, `a {\n\tcolor: pink; /* c */\n\n\n}\n`],
	[`nested-free-semicolon-lines`, `a {\n\tb {}\n\n\n;\n\n\n}\n`],
	[`bodiless-lines`, `a {\n\t@content;\n\n\n}\n`],
	[`no-semicolon-lines`, `a {\n\tcolor: pink\n\n\n}\n`],
	[`no-value-run`, `a {\n\tx:  }\n`],
	[`custom-no-value-run`, `a {\n\t--x:  }\n`],
]
