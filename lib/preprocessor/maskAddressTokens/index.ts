...
import { EVERY_CHARACTER_BUT_A_LINE_FEED, EVERY_PARENTHESIS } from "../../regexps.ts"
...
export function maskAddressTokens (copy: string, text: string, node: Node, result?: PostcssResult): string {
	...
	for (let { start, end } of spans) {
		...
		pieces.push(copy.slice(index, start + 1), token.slice(1, closed ? -1 : undefined).replaceAll(EVERY_CHARACTER_BUT_A_LINE_FEED, `?`))
