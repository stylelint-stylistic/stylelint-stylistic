...
/** A Windows line break. Read by `max-empty-lines` to tell a pair from a bare break; telling the two apart is a question about the spelling and is meant, while the set the rule can see at all is not — it counts no empty line whatever in a file broken with bare carriage returns or with form feeds, and the census of #250 holds it. Read by `indentation` as well, to ask of the line feed a search has stopped at whether a carriage return in front of it belongs to the same break — a question about the pair itself, so the narrowness is the whole of what is wanted there. */
export const CRLF = /\r\n/u

/** A run of Windows line breaks. Read by `max-empty-lines`, and narrow for the reason {@link CRLF} is. */
export const CRLF_RUN = /(?:\r\n)+/u

/** A line feed or a carriage return, the two breaks every syntax reads as one. Read by `linebreaks`, which asks which of those two characters a file ends its lines with, and by the scan behind `endsWithInlineComment`, wherever the syntax that spelled the text reads no line in a form feed — Less normalises the line endings of a file before parsing it and does not, and {@link LINE_BREAK} is the reading Sass has. A form feed is neither of these two characters, so this is one of the five narrow readings in this file that are narrow on purpose. */
...
/** Every line break of a text that no line feed ends: a carriage return with none behind it, and a form feed. Read by `indentation`, which writes each of them as a line feed into the copy it hands `style-search`, so that one target finds every break of the text at the very index the file holds it — the copy is as long as the text, and a Windows pair, whose line feed the target finds already, is left as it stands. The name matches exactly what it is named for, so it is narrow on purpose the way {@link FORM_FEED} is, and the fifth of the five names in this file that are. */
export const EVERY_BARE_CR_OR_FORM_FEED = /\r(?!\n)|\f/gu
...
/** Every line break of a text spelled with a carriage return or a line feed, a Windows pair counting as one so that a replacement never lands between its two characters. Read by `linebreaks`, which writes each of them as the break its option asks for, one character or two — a form feed is neither of the two that rule reads, so this name is narrow the way {@link CARRIAGE_RETURN} and {@link CR_OR_LF} are, on purpose, and the third of the five names in this file that are. */
...
/** The spaces and tabs every line that holds content is indented by. Where a line begins is named here rather than left to the `m` flag, whose set is a different one: it begins a line after the two separators of Unicode, which end a line to JavaScript and to no stylesheet, and begins none after a form feed, which ends one to CSS and to Sass. */
...
/** A form feed, the one break the two syntaxes disagree about: Sass ends an inline comment on one, and Less, which normalises the line endings of a file before parsing it, reads no line in one at all. Read by the probe of `readsInlineComments`, which hands a syntax a comment with a form feed in the middle of it and asks whether the comment ran on past the character — the answer telling the one language from the other. Narrow on purpose, then, and the fourth of the five names in this file that are. */
export const FORM_FEED = /\f/u
...
/** A line break, each of the three one character long — a form feed among them, since Sass ends an inline comment on one. Read by the scan behind `endsWithInlineComment` wherever the syntax that spelled the text is one that does, {@link CR_OR_LF} being the reading Less has. */
