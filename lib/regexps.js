...
/** Every form feed of a text, the one break the two syntaxes disagree about: Sass ends an inline comment on one and Less, which normalises the line endings of a file before parsing it, reads no line in one at all. Read by the closing guard of `function-parentheses-space-inside` and `function-parentheses-newline-inside`, which asks its two questions once under the breaks both syntaxes read and once with a form feed written in as a line feed, so that each answer describes a reading some syntax actually has. Narrow on purpose, then, and the third of the three names in this file that are. */
