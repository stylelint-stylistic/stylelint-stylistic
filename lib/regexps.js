...
/** The first line break of a text, of either spelling, captured so that a replacement can write it back. Read by `addEmptyLineAfter` to put a break beside the one already standing; that a bare carriage return and a form feed are not among the spellings it knows is not deliberate, and the census of #250 holds it. */
...
/** A line feed or a carriage return, the two breaks every syntax reads as one. Read by `linebreaks`, which asks which of those two characters a file ends its lines with — a form feed is neither, so this is one of the three narrow readings in this file that are narrow on purpose. */
export const CR_OR_LF = /[\r\n]/u

/** Two line breaks with nothing but horizontal whitespace between them. Read by `hasEmptyLine`; that neither break may be a bare carriage return or a form feed is not deliberate and is half of #245. */
export const EMPTY_LINE = /\n[\r\t ]*\n/u

/** Every break that closes an inline comment anywhere but in both syntaxes at once: a form feed, which Sass ends a comment on and Less does not, and the two separators of Unicode, which one reading of Less's own parser ends one on though neither syntax the plugin runs under does. The question they are counted for is the one whose wrong answer lets a breaking fix through, so a break is counted here as soon as anything reads it as one — the form feed because a fix does break through it, the separators because nothing but the value parser's own reading of what a space is keeps them from it. Narrow on purpose, then, and the third of the three names in this file that are. */
...
/** Every run of line breaks that leaves an empty line behind it. Read by `removeEmptyLinesAfter`, and narrow for the reason {@link EMPTY_LINE} is. */
...
/** A line break of either spelling, Unix or Windows. Read by `addEmptyLineAfter`, and narrow for the reason {@link CAPTURED_LF_OR_CRLF} is. */
...
/** The name a call is opened by, read back from its parenthesis, and the empty string where nothing nameable stands there. A name is the whole run of name characters standing in front of the parenthesis, and it is a name only where that run opens on a letter or an underscore, whatever hyphens stand in front of that, and closes on a letter, an underscore or a digit: the `-` of `-(@a * 2)`, the `2-` of `2-(1)` and the `2and` of `2and(min-width:1px)` name nothing, and none of the three opens a call. It reads less than an identifier of CSS, which may close on a hyphen, holds code points outside ASCII and opens on anything at all behind a `--`, and less again than Less, which names a call by `%`; and a run of name characters is not an identifier at all, an escape spelling one character with several, so `fo\6f(` is a call to `foo` whose run reads `6f` and names nothing, while a run standing behind what the class cannot see is read from where the class begins, `日本and(` naming `and`. #252 carries the five shapes read as no name. A run opening on a digit is refused on purpose and not counted among them: `2px(` is a dimension in front of a parenthesis rather than a call. */
export const TRAILING_FUNCTION_NAME = /(?:(?<![\w-])-*[A-Za-z_](?:[\w-]*\w)?)?$/u
