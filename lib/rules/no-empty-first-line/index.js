...
import { OPENS_WITH_CR_OR_LF } from "../../regexps.js"
...
function rule (primary) {
	return (root, result) => {
		...
		if (OPENS_WITH_CR_OR_LF.test(rootString)) {
			report({
				...
				fix () {
					...
					root.first.raws.before = root.first.raws.before.replace(OPENS_WITH_CR_OR_LF, ``)
				},
			})
		}
	}
