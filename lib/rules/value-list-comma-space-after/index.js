...
function rule (primary) {
	...
	return (root, result) => {
		...
		valueListCommaWhitespaceChecker({
			...
			// name, and nothing this rule could write would reach it. A comma opening the
			// value is the value's first character, and the whitespace behind it belongs
			// to the value like any other, so the boundary takes that one in.
		...
		if (fixData) {
			for (let [decl, commaIndices] of fixData.entries()) {
				for (let index of commaIndices.toSorted((a, b) => b - a)) {
					...
					if (primary.startsWith(`always`)) afterValue = afterValue.replace(/^\s*/u, ` `)
					else if (primary.startsWith(`never`)) afterValue = afterValue.replace(/^\s*/u, ``)
