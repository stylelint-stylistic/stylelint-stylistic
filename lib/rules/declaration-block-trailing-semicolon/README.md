...
The semicolon `"always"` writes behind a declaration is written with whatever the `declaration-block-semicolon-newline-before` and `declaration-block-semicolon-space-before` rules ask to stand in front of one, wherever the configuration lists either of them, so that it is spelled like the block's other semicolons whatever order the rules are listed in; the one it writes behind an at-rule gets whatever `at-rule-semicolon-space-before` asks for.

The [`fix` option](https://stylelint.io/user-guide/options#fix) can automatically fix the problems reported by this rule, except the ones a comment leaves the semicolon nowhere to stand in: behind a value or a set of parameters that a `//` comment runs to the end of, `"always"` writes the semicolon in front of the comment, which keeps its line behind it, unless no line feed closes the comment in front of the closing brace, a flag stands behind it, or the code ends on a backslash escaping nothing, where it writes none; and `"never"` takes none away from behind a bodiless at-rule or a custom property that a comment closes the block behind, since that is a semicolon PostCSS writes back whatever the stylesheet asks for.
