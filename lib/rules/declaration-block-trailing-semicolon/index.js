 ...
 * @param {import('postcss').Node} node - The node.
 * @returns {{ start: number, end: number }} The two offsets.
 */
 ...
 * @param {import('postcss').ChildNode} node - The node closing the block.
...
function rawsBehind (node) {
	...
	/** @type {{ owner: import('postcss').Node, key: string, start: number, text: string }[]} */
	let raws = []
 ...
 * @param {import('postcss').ChildNode} node - The node the semicolon stands behind.
 * @param {'always' | 'never'} primary - The primary option.
 ...
 * @type {import('stylelint').RuleBase<'always' | 'never', { ignore?: 'single-declaration' | 'single-declaration'[] }>}
 */
