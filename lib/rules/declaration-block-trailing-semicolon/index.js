...
function rule (primary, secondaryOptions) {
	return (root, result) => {
		...
		function checkLastNode (node) {
			...
			// Under `never` the warning stands on the semicolon the rule is named for, counted in the file. Under `always` the flag says no semicolon closes the node, so the warning stands at the end of that node, where one is to be written — and that end is measured in the printed copy of the node, which is wider than the file wherever the node carries an inline comment. The width is #139, and it is left where it stands
			let problemIndex = trailingSemicolon ?? node.toString().trim().length - 1
