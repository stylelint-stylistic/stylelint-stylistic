		...
		if (fixData) {
			for (let [atRule, commaIndices] of fixData.entries()) {
				...
				for (let index of commaIndices.toSorted((a, b) => b - a)) {
					...
					if (primary.startsWith(`always`)) params = (/^\s*[\n\r\f]/u).test(afterComma) ? beforeComma + afterComma.replace(/^[^\S\n\r\f]*/u, ``) : beforeComma + context.newline + afterComma
					else if (primary.startsWith(`never`)) params = beforeComma + afterComma.replace(/^\s*/u, ``)
				}
