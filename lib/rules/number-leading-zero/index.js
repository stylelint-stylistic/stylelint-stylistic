	...
	return (root, result) => {
		...
		function check (node, value) {
			...
			valueParser(value).walk((valueNode) => {
				...
				// Check leading zero
				if (primary === `always`) {
					let match = (/(?:\D|^)(\.\d+)/u).exec(valueNode.value)

					if (match === null || match[0] === null || match[1] === null) return

					// The regexp above consists of 2 capturing groups (or capturing parentheses). We need the index of the second group. This makes sanse when we have "-.5" as an input for regex. And we need the index of ".5".
				...
				if (primary === `never`) {
					let match = (/(?:\D|^)(0+)(\.\d+)/u).exec(valueNode.value)

					if (match === null || match[0] === null || match[1] === null || match[2] === null) return

					// The regexp above consists of 3 capturing groups (or capturing parentheses). We need the index of the second group. This makes sanse when we have "-00.5" as an input for regex. And we need the index of "00".
