...
function rule (primary) {
	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			// the raw holding it is the text the fix writes
			let writesImportant = primary.startsWith(`always`) ? Boolean(decl.important) : Boolean(decl.raws.important)
			// `postcss-scss` rewrites every `//` comment of a value into a block comment, keeps the
			// spelling of the file in a copy of its own and prints that copy: the comment a fix runs
			// into is the one the printed copy spells
			let printedValue = (decl.raws.value && decl.raws.value.scss) || value
			// The semicolon goes right after this text, and the whitespace run the fix cuts into ends it.
			// Where an inline comment stands there, the line break that run begins with is what closes
			// the comment, so either option would take the semicolon into the comment's text: neither
			// can be satisfied, so leave the declaration alone and let the warning stand
			let isFixable = !endsWithInlineComment(writesImportant ? decl.raws.important || ` !important` : printedValue)
						...
						fix: isFixable
							? () => {
								if (primary.startsWith(`always`)) {
									// The raw is kept rather than written anew, so that a comment,
									// and any other layout standing in front of the flag, survives the fix
