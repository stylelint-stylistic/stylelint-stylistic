...
testRule({
	...
	reject: [
		...
		},
		{
			...
			description: `a form feed closing the comment, which Sass reads a line in: the semicolon stands in code behind it and the space the option asks for goes in`,
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/333
			description: `the same form feed under a syntax that normalises the line endings of a file before parsing it and reads no line in one, so the semicolon stands in the comment's text and the value is left alone`,
			code: `a { b: 1px // c\f\t2px; }`,
			fixed: `a { b: 1px // c\f\t2px; }`,
			line: 1,
			column: 20,
