...
import { LEADING_WHITESPACE, LINE_BREAK } from "../../regexps.js"
...
import { movesEndIntoInlineComment } from "../../utils/movesEndIntoInlineComment/index.js"
 ...
 */
function findInlineCommentSpansAfterEdits (declValue, edits, spellsInlineComments) {
 ...
 * @param {import('../../utils/readsInlineComments/index.js').InlineCommentReading} reading - What the syntax the value was spelled in makes of a comment opened by a double slash.
 * @returns {boolean} True where a reading has the character move into a comment.
 */
function movesIntoComment (declValue, characterIndex, emptied, reading) {
...
function findFirstCharacterIndex (declValue, firstIndex) {
	...
	return firstIndex + /** @type {RegExpMatchArray} */ (declValue.slice(firstIndex).match(LEADING_WHITESPACE))[0].length
}
 ...
 * @param {{ declValue: string, valueNode: FunctionNode, openingIndex: number, checkBefore: string, checkAfter: string, firstIndex: number, measured: number[][], reading: import('../../utils/readsInlineComments/index.js').InlineCommentReading }} read - What the walk has read of the function, and the value it was read from.
 * @returns {{ isOpeningFixable: boolean, isClosingFixable: boolean }} Whether each of the two fixes may be written.
 */
function getNeverFixability ({ declValue, valueNode, openingIndex, checkBefore, checkAfter, firstIndex, measured, reading }) {
	...
	for (let node of valueNode.nodes) {
		...
		if (span) {
			...
			if (node.sourceEndIndex > span.end) {
				...
				// The run may be empty, so the pattern matches every text
				let whitespace = /** @type {RegExpMatchArray} */ (overrun.match(LEADING_WHITESPACE))[0]
