...
/** The line breaks a value is counted in, each of them one character long. A form feed is among them: Sass ends an inline comment on it, so code can follow one on the line a line feed counts. */
let lineBreak = /[\n\r\f]/u

/** Every break that closes an inline comment anywhere but in both syntaxes at once: a form feed, which Sass ends a comment on and Less does not, and the two separators of Unicode, which one reading of Less's own parser ends one on. A wrong answer to the question they are counted for lets a breaking fix through, so a break is counted there as soon as anything reads it as one. */
...
function printedBefore (printedValue, declValue, index) {
	...
	// of the file in a copy of its own and prints that copy. No rewriting crosses a line break, since
	// an inline comment runs to the end of its line, so the two copies hold the same lines, and
	// nothing can follow such a comment on its line either, so a position keeps its column as well.
	// Every break CSS knows ends a line here, since a line is only counted, and each of them is one
	// character long, so joining the lines back with a line feed keeps every position where it was.
	// A comment whose own text holds `/*` or `*/` is rewritten into several block comments rather
	// than one, and a position inside such a line is then read a few characters short — which can
	// only make the rule decline a fix, never let one through, and reaches no output while the
	// syntax discards the write anyway (#115). Repairing it belongs with that issue, which takes
	// this mapping out of the way altogether by handing the rule the copy it prints.
 ...
 */
function beforeClosingString (decl, declValue, valueNode) {
	let closingIndex = valueNode.sourceEndIndex - 1
	// The fix empties the whitespace the function keeps in front of the parenthesis and then every
	// whitespace node behind it, stepping over the comments among them and stopping at anything else,
	// so all of that closes up against the parenthesis on one line
	...
	// A comment already holding the parenthesis can take nothing new into itself, whatever the fix
	// does with the whitespace, so such a value is left to the fix as it always was. The parenthesis
	// is written back on the end for the asking, since the whitespace in front of it would otherwise
	// be read as room a fix is about to write in, which is the question the other call asks.
	// This one is answered the other way round as well: a comment wrongly taken for one holding the
	// parenthesis lets a breaking fix through, while the other call only holds a safe fix back, so
	// every break either syntax closes a comment on counts here, and only the breaks both of them
	// close one on count there. Each syntax reads a break the other does not — Sass a form feed,
	// Less the two separators of Unicode — so they are written as line feeds for the asking.
	if (endsWithInlineComment(`${standing.replaceAll(closingBreak, `\n`)})`)) return ``

	// Closing the gap up can also bring a slash against a comment's own and open a comment that was
	// never there, so what the fix would leave behind is what has to be looked at, not what stands.
	// The parenthesis goes on the end here as well: the whitespace of the gap is gone by then, and
	// whatever whitespace the fix does not empty — the value parser counts as space only what stands
	// below the blank, so a separator of Unicode is a word to it — would otherwise be trimmed away
	// with the line break it holds and the comment in front of it read as still open
	return `${kept}${standing.slice(kept.length).replaceAll(/\s+/gu, ``)})`
}
...
function rule (primary, _secondaryOptions, context) {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			// A double slash opens a comment that runs to the end of its line, and the value parser
			// knows nothing of the kind: what such a comment holds comes back as ordinary nodes
			let inlineComments = findInlineCommentSpans(declValue, endsInlineCommentOnFormFeed(decl))
			...
			parsedValue.walk((valueNode) => {
				...
				if (isMultiLine && primary === `never-multi-line` && checkBefore !== ``) {
					// The first argument goes right after this text, and the whitespace run the fix
					// empties ends it. Where an inline comment stands there, the line break that run
					// holds is what closes the comment, so the option cannot be satisfied without taking
					// that argument, and everything the value has left, into the comment's text: leave
					// the value alone and let the warning stand. A function holding nothing but comments
					// is asked the same about its own closing parenthesis.
					let isOpeningFixable = !endsWithInlineComment(declValue.slice(0, firstIndex))
				...
				if (isMultiLine && primary === `never-multi-line` && checkAfter !== ``) {
					// The parenthesis goes right after this text, and the whitespace run the fix empties
					// ends it. Where an inline comment stands there, the line break that run holds is
					// what closes the comment, so the fix would take the parenthesis, and the semicolon
					// behind it, into the comment's text: leave the value alone and let the warning stand
					let isFixable = !endsWithInlineComment(beforeClosingString(decl, declValue, valueNode))
 ...
 *
 * A comment may stand between the parenthesis and the line break the `always` options ask for, so it
 * is walked past and the whitespace behind it counted. `postcss-value-parser` knows nothing of a
 * comment opened by a double slash and hands out two division signs and a word where one stands, so
 * the nodes that make up such a comment are placed against the spans it occupies instead.
 *
 * Where the function holds nothing but comments and whitespace, the first significant thing is its
 * own closing parenthesis, which is what the `never` fix would then pull up.
...
function getCheckBefore (valueNode, declValue, inlineComments) {
	...
	for (let node of valueNode.nodes) {
		...
		if (span) {
			// The value parser hangs the whitespace behind a node on that node rather than emitting a
			// node of its own for it, so the break closing the comment reaches past the comment's end
			// whenever the last thing it spells is a division sign, a colon or a comma. It is whitespace
			// of the value like any other, and the walk would drop it with the node it hangs on.
