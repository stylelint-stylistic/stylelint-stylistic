...
import { EVERY_FORM_FEED, EVERY_WHITESPACE_RUN, LINE_BREAK } from "../../regexps.js"
...
/**
 * Asks whether an inline comment holds a closing parenthesis, under each of the two readings of a line break a syntax of this plugin has.
 *
 ...
 *
 * The two separators of Unicode are read by neither, and they need no reading of their own. A fix here writes over whitespace, and the value parser counts as space only what stands below the blank, so a separator survives every fix this guard stands in front of — whatever a reading makes of it, it makes the same of it before the fix and after, and it can never be what carries the parenthesis into a comment.
 *
 * The parenthesis is written back on the end for the asking. `endsWithInlineComment` reads trailing whitespace as room a fix is about to write in, which is the question its other callers ask and the opposite of this one, and a separator standing at the end would otherwise be trimmed away with the line break it holds and the comment in front of it read as still open.
 * @param {string} before - The text the parenthesis stands behind.
 * @param {boolean} spellsInlineComments - False where the syntax the value was spelled in writes no comment with a double slash.
 * @returns {{ shared: boolean, formFeed: boolean }} Whether a comment holds the parenthesis under the breaks both syntaxes read, and under those with a form feed among them.
 */
function readsClosingInsideComment (before, spellsInlineComments) {
	return {
		shared: endsWithInlineComment(`${before})`, spellsInlineComments),
...
function movesClosingIntoComment (decl, declValue, valueNode, spellsInlineComments) {
	...
	let keptText = printedBefore(printedValue, declValue, emptiedIndex)
	...
	let standing = readsClosingInsideComment(standingText, spellsInlineComments)
	let fixed = readsClosingInsideComment(fixedText, spellsInlineComments)
		...
		root.walkDecls((decl) => {
			...
			parsedValue.walk((valueNode) => {
				...
				if (isMultiLine && primary === `never-multi-line` && checkAfter !== ``) {
					...
					let isFixable = !movesClosingIntoComment(decl, declValue, valueNode, spellsInlineComments)
