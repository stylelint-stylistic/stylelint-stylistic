...
function getNeverFixability ({ declValue, valueNode, openingIndex, checkBefore, checkAfter, firstIndex, measured, spellsInlineComments }) {
	...
	// The fix reaches less far than the walk that measured the whitespace behind the opening parenthesis: that one steps over the nodes an inline comment is spelled with, and the fix stops at the first of them. What it does not reach stays, the option stays violated, and Stylelint calls the problem solved and hands the next run the same one (#285). So the fix is written only where it empties every stretch that was measured. The safety question is then put over what the fixer empties rather than over what was measured, the first being the honest set to ask about; the two answer alike, since with this question answered they differ only by whitespace standing inside a comment's span, and no such whitespace can hold the break that closes that comment — the span ends at that break.
