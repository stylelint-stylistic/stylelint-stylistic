...
function rule (primary, _secondaryOptions, context) {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			// A double slash spells a comment only where the syntax says one, and a file of plain CSS
			// spells none: the pair in `myurl(//a)` is code there, and taking it for a comment would
			// silence everything standing behind it on the line
