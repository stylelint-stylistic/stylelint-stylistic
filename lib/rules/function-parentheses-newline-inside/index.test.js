...
testRule({
	...
	reject: [
		...
		},
		{
			...
			description: `the same form feed the Less twin of this file is fixed on, which Sass does end the comment on: that reading is the one and only reading with the argument outside the comment now and inside it after the fix, and the value is left alone on its word`,
			code: `a { t: foo(1px // c) calc(\f2px/*x\n*/); }`,
			fixed: `a { t: foo(1px // c) calc(\f2px/*x\n*/); }`,
			line: 1,
			column: 27,
			message: messages.rejectedOpeningMultiLine,
		},
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/281
			description: `the same comment with the first argument standing inside it, so that emptying the whitespace moves that argument nowhere it was not: the closing parenthesis is a line down and cannot be pulled up, and the warning about it stands where it is`,
			code: `a { t: foo(1px // c) calc( 2px\n ); }`,
			fixed: `a { t: foo(1px // c) calc(2px\n ); }`,
			warnings: [
				{
					line: 1,
					column: 27,
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/281
			description: `a block comment standing between the break that closes the inline one and the first argument: emptying the whitespace on either side of it takes that argument into the comment, so the value is left alone`,
			code: `a { t: foo(1px // c) calc(\n/*b*/ 2px); }`,
			fixed: `a { t: foo(1px // c) calc(\n/*b*/ 2px); }`,
			line: 1,
			column: 27,
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/281
			description: `a form feed in that whitespace, which Less reads no line in: the comment runs to the break inside the block comment behind the argument, so it holds the argument before the fix as after it`,
			code: `a { t: foo(1px // c) calc(\f2px/*x\n*/); }`,
			fixed: `a { t: foo(1px // c) calc(2px/*x\n*/); }`,
			line: 1,
			column: 27,
			message: messages.rejectedOpeningMultiLine,
		},
