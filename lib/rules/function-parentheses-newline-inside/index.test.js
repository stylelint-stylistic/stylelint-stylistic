...
testRule({
	...
	accept: [
		...
		},
		{
			...
			description: `a form feed, which Sass ends the comment on and Less does not: the comment runs to the end of the value here, and the parenthesis the parser closed the call on stands in its text`,
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/132
			description: `the same form feed with a line break behind it, which is the break Less does close the comment on: the parenthesis stands outside the comment by that break alone, and the fix would take it away`,
			code: `a { transform: translate(1px,\n2px // keep me\f d\n); }`,
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/132
			description: `a separator standing in front of the form feed the comment ends on: Sass, which the file is written in, has the parenthesis outside the comment now and inside it once the whitespace in front of it is gone, the separator being no whitespace to the value parser and no break to any syntax`,
			code: `a { transform: translate(1px,\n2px //c\u2028\f); }`,
			fixed: `a { transform: translate(1px,\n2px //c\u2028\f); }`,
			line: 2,
			column: 9,
		...
		},
		{
			...
			description: `a form feed closing the comment in front of the call, and a line break where the first argument opens: Sass ends the comment on that form feed, so the argument stands outside it before the fix as after, and the break goes`,
		...
		},
		{
			...
			description: `the same form feed with a block comment standing first inside the call: Sass has the argument outside the comment on either side of the fix, and the reading Less has, which this file is not written in, is no longer asked about it`,
			code: `a { t: 1px // c\fcalc(\n/*b*/ 2px); }`,
			fixed: `a { t: 1px // c\fcalc(/*b*/2px); }`,
			line: 1,
			column: 22,
			message: messages.rejectedOpeningMultiLine,
		},
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/281
			description: `the same form feed with a space where the first argument opens and a line break in front of the closing parenthesis: neither of the two stands inside the comment Sass has closed, and both fixes are written`,
		...
		},
		{
			...
			description: `a call holding nothing but a block comment behind that form feed, so that both questions are about its closing parenthesis: each fix is weighed on its own and the two are weighed again over what they empty between them, and Sass has closed the comment on the form feed, so neither carries the parenthesis into one`,
			code: `a { t: 1px // c\fcalc(\n/*b*/\n); }`,
			fixed: `a { t: 1px // c\fcalc(/*b*/); }`,
			warnings: [
				{
					line: 1,
					column: 22,
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/312
			description: `an argument standing behind that form feed rather than a comment, which Sass has in code like the parenthesis: the two fixes are weighed together over the whitespace they empty between them, and both are written`,
			code: `a { t: 1px // c\fcalc(\n2px\n); }`,
			fixed: `a { t: 1px // c\fcalc(2px); }`,
			warnings: [
				{
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/312
			description: `an argument in front of that block comment, whose own text a line break is written inside: Sass closed the inline comment on the form feed, so the argument and the parenthesis stand in code on either side of the fix, and closing the gap up carries no character of the value into a comment`,
