...
function rule (primary) {
	return (root, result) => {
		...
		if (root.raws.after && root.raws.after.trim().length !== 0) {
			...
			// fix
			if (fixSemiIndices.length) root.raws.after = removeIndices(rawAfterRoot, fixSemiIndices)
		}
		...
		root.walk((node) => {
			...
			if (node.raws.before && node.raws.before.trim().length !== 0) {
				...
				// fix
				if (fixSemiIndices.length) node.raws.before = removeIndices(rawBeforeNode, fixSemiIndices)
			}
			...
			if (typeof node.raws.after === `string` && node.raws.after.trim().length !== 0) {
				...
				// fix
				if (fixSemiIndices.length) node.raws.after = removeIndices(rawAfterNode, fixSemiIndices)
			}
			...
			if (typeof node.raws.ownSemicolon === `string`) {
				...
				// fix
				if (fixSemiIndices.length) node.raws.ownSemicolon = removeIndices(rawOwnSemicolon, fixSemiIndices)
			}
		})
		...
		function removeIndices (str, indices) {
			for (let index of indices.reverse()) str = str.slice(0, index) + str.slice(index + 1)
