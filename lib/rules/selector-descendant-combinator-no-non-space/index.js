...
function rule (primary) {
	return (root, result) => {
		...
		root.walkRules((ruleNode) => {
			 ...
			 * @param {string} text - The text as the parsed selector spells it.
			 * @param {number} rawIndex - The index that text stands at in the parsed selector.
			 * @returns {string} The same stretch, spelled as the file spells it.
			 */
			function sourceSpelling (text, rawIndex) {
			...
			let fixedSelector = parseSelector(selector, result, ruleNode, (fullSelector) => {
				// Everything below rests on one thing: the text of a node stands in the file where
				// its `sourceIndex` says it does. A selector the tree does not stand for is passed
				// over, since the text of a warning, the position it is reported at and the
				// selector a fix writes back would all be measured against a rendering the file
				// does not hold.
				...
				fullSelector.walkCombinators((combinatorNode) => {
					...
					if (!isDescendant) {
						...
						// something valid, so the problem is reported and the code left as it was. The
						// warning names the text it is about, and the text a node stands for is read
						// out of the raw, so it is read back out of the copy the file spells before it
...
function standsForSource (selectorTree, selector) {
	...
	selectorTree.walkCombinators((combinatorNode) => {
		...
		let [, whitespace, group] = (/^(\s*)([\s\S]*)$/u).exec(combinatorNode.raws.value ?? combinatorNode.value)
