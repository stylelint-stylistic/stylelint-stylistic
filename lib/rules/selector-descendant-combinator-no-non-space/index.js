...
function rule (primary) {
	return (root, result) => {
		...
		root.walkRules((ruleNode) => {
			...
			/** @type {import('../../utils/typeGuards/index.ts').SyntaxRaw | undefined} */
			...
			fullSelector.walkCombinators((combinatorNode) => {
				 ...
				 * @param {{ value: string, index: number, isComment: boolean, closesInlineComment: boolean }} segment - The run, as {@link splitAtComments} cut it.
				 */
...
function standsForSource (selectorTree, selector) {
	...
	selectorTree.walkCombinators((combinatorNode) => {
		...
		let [, whitespace, group] = /** @type {RegExpExecArray} */ (LEADING_WHITESPACE_AND_REST.exec(combinatorNode.raws?.value ?? combinatorNode.value))
 ...
 * @param {import('../../utils/findSelectorInlineComments/index.ts').InlineComment[]} inlineComments - The inline comments of the selector.
