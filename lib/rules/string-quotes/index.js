 ...
 * `image-url(` and `@{prefix}url(` end in the same four characters while being ordinary calls,
 * whose arguments may hold a comment — and the parentheses are counted rather than searched for,
 * since an interpolated address brings its own. A parenthesis that is escaped, quoted or
 * commented out is none, and a token left open is taken for no token at all, so that a comment
 * standing behind it is still seen.
