...
function rule (primary, secondaryOptions) {
	...
	return (root, result) => {
		...
		// A double slash opens a comment only where the syntax says it does. In plain CSS it is
		// part of a value — an address, most often — and reading it as a comment would silence
		// every string behind it on the line. The two spellings are identical, so the text cannot
		// answer this and the syntax has to.
		let doubleSlashOpensComment = syntaxHasInlineComments(result.opts && result.opts.syntax)
		...
		function checkDeclOrAtRule (node, value, getIndex) {
			...
			let inlineCommentSpans = doubleSlashOpensComment ? findInlineCommentSpans(value) : []
...
/** @type {WeakMap<object, boolean>} The verdict of {@link syntaxHasInlineComments}, per syntax. */
let inlineCommentSyntaxes = new WeakMap()

/**
 * Asks a syntax whether it reads a double slash as the opening of a comment, by handing it one
 * and looking at what comes back. Naming the syntaxes that do would miss every custom one, and a
 * syntax passed as an object has no name to go by in the first place.
 * @param {any} syntax - The syntax the stylesheet was parsed with.
 * @returns {boolean} True if the syntax parses an inline comment as such.
 */
function syntaxHasInlineComments (syntax) {
	...
	try {
		syntax.parse(`a {}\n// comment\n`, { from: undefined }).walkComments((comment) => {
			if (comment.inline || comment.raws.inline) verdict = true
		})
	}
	catch {
		// A syntax that cannot parse an inline comment at all does not have them
	}
...
function findInlineCommentSpans (text) {
	...
	while (index < text.length) {
		...
		if (character === `"` || character === `'`) {
			...
			let behindUrl = skipBareUrl(text, index)
...
/**
 * Skips an unquoted `url()` token, whose address carries its double slashes as ordinary
 * characters. A quoted one is left alone, since the string branch of the scan already covers it
 * and the rule has a quotation mark to say something about. The name has to stand on its own —
 * `image-url(` ends in the same four characters and is an ordinary call, whose arguments may hold
 * a comment — and the parentheses are counted, since an interpolated address brings its own.
 * @param {string} text - The string being scanned.
 * @param {number} openIndex - The index the token would start at.
 * @returns {number} The index behind the closing parenthesis, or the given one if no unquoted
 *   `url()` starts there.
 */
function skipBareUrl (text, openIndex) {
	...
	let addressIndex = openIndex + 4

	while (addressIndex < text.length && (/\s/u).test(text[addressIndex])) addressIndex += 1

	if (text[addressIndex] === `"` || text[addressIndex] === `'`) return openIndex
	...
	return depth > 0 ? text.length : index
}
