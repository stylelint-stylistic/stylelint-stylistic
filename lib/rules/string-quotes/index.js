...
/** @typedef {import('../../utils/findInlineCommentSpans/index.ts').InlineCommentSpan} InlineCommentSpan */

/**
 * Specifies single or double quotes around strings.
 * @type {import('stylelint').RuleBase<'single' | 'double', { avoidEscape?: boolean }>}
 */
function rule (primary, secondaryOptions) {
	/** @type {typeof SINGLE_QUOTE | typeof DOUBLE_QUOTE} */
	let correctQuote = primary === `single` ? SINGLE_QUOTE : DOUBLE_QUOTE

	/** @type {typeof SINGLE_QUOTE | typeof DOUBLE_QUOTE} */
	let erroneousQuote = primary === `single` ? DOUBLE_QUOTE : SINGLE_QUOTE
