...
// A rule fixing the same value stands beside this one in a real run, and `postcss-scss` gives that value a copy apiece: whichever rule writes to one of them and leaves the other where it was takes away what the pair says between them about the comments in the value.
describe(`${ruleName} beside a rule writing the raw of the same value`, () => {
	it(`fixes a value whose raw another rule has written a prefix into`, async () => {
		// `declaration-colon-space-after` puts the space it asks for into the raw of the value
	...
	it(`fixes a value whose raw another rule has taken the line break of a comment out of`, async () => {
		// `value-list-comma-newline-after` takes every line break of the raw away, the one closing the comment among them, and the raw then says nothing about where that comment ends
		expect(await fix(`a {\n  b: 'x', // c\n     'y';\n}\n`, {
	...
	it(`fixes a value whose raw another rule has written inside`, async () => {
		// The hexadecimal colour keeps its case, since that rule writes its own fix into the raw, which is not the copy this syntax prints
		expect(await fix(`a {\n  b: #FFF 'x', // c\n     'y';\n}\n`, {
