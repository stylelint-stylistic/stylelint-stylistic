...
function rule (primary) {
	...
	return (root, result) => {
		...
		declarationBangSpaceChecker({
			...
			isFixable: (decl, index) => !endsWithInlineComment(declarationString(decl).slice(0, index), readsInlineComments(decl, result)),
			fix: (decl, index) => {
				...
				if (bangIndex < value.length) {
					...
					// The bang stands neither in the value nor in the `!important` the syntax printed, so it belongs to a spelling this rule has no place to write back to
					return false
				}
				...
				if (primary === `always`) {
					setFixed(`${targetBefore.replace(TRAILING_WHITESPACE, ``)} ${targetAfter}`)
