...
testRule({
	...
	accept: [
		...
		},
		{
			...
			code: `a:not(b\r, c ) {}`,
			description: `the same list broken with a carriage return, which ends a line as readily`,
		},
		{
			code: `a:not(b\f, c ) {}`,
			description: `the same list broken with a form feed`,
		},
		{
			code: `.foo {
	&:has(
	...
	reject: [
		{
			...
			code: `a:not( b\r, c ) {}`,
			fixed: `a:not(b\r, c ) {}`,
			description: `a list broken with a carriage return: the opening space goes, the closing is the multi-line list's own`,
		...
		},
		{
			code: `a:not( b\f, c ) {}`,
			fixed: `a:not(b\f, c ) {}`,
			description: `the same, broken with a form feed`,
			message: messages.rejectedOpening,
			line: 1,
			column: 7,
		},
