 ...
 *
 * The mirror of `setFirstNodeSpaceBefore`, and the side a comment actually reaches:
 * a trailing comment is folded into the raws of the node in front of it whenever
 * whitespace separates the two, which is what makes the raw the printed one here.
 * With nothing between them it is a node of its own instead, and no raw appears.
 * Nothing ever folds into the raws of a pseudo-class's first node — the parser does
 * write `raws.spaces.before`, but on a combinator with a comment in front of it,
 * never here — so the mirror has no reproducer of its own.
 * @param {import('postcss-selector-parser').Container} node - The container node.
 * @param {string} value - The space value to set.
 * @returns {void}
 */
