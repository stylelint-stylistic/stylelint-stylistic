...
function rule (primary) {
	return (root, result) => {
		...
		root.walkRules((ruleNode) => {
			...
			// `postcss-scss` rewrites every inline comment of a selector into a block comment in the raw it parses, keeps the source spelling beside it and prints that one, so the two strings drift apart by two characters per comment: a position counted in the raw points past the character it is about, and a fix written there never reaches the file. Such a selector is passed over rather than reported at a drifting position and left unfixed behind a clean pass, until the rule reads and writes the copy that is printed, which is #163.
