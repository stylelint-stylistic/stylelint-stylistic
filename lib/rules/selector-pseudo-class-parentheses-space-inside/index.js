	...
	return (root, result) => {
		...
		root.walkRules((ruleNode) => {
			...
			selectorTree.walkPseudos((pseudoNode) => {
				...
				// A line ends on a carriage return and on a form feed as readily as on a line feed — `findSelectorInlineComments` reads all three already — so a list broken with any of them is a multi-line list.
...
function setSpaceBefore (target, value) {
	...
	if (raw !== undefined) target.raws.spaces.before = value + raw.replace(LEADING_WHITESPACE_RUN, ``)
}
...
function setSpaceAfter (target, value) {
	...
	if (raw !== undefined) target.raws.spaces.after = raw.replace(TRAILING_WHITESPACE_RUN, ``) + value
}
