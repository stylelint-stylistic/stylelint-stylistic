...
import { LEADING_WHITESPACE_OR_BLOCK_COMMENT, LEADING_WHITESPACE_RUN, LINE_BREAK, TRAILING_WHITESPACE_RUN, WHITESPACE } from "../../regexps.js"
 ...
 * @typedef {import('postcss-selector-parser').Node & { raws?: { spaces?: Partial<import('postcss-selector-parser').Spaces>, value?: string } }} NodeWithRaws
 */
 ...
 * @param {import('postcss-selector-parser').Node | import('postcss-selector-parser').Container | undefined} node - The node to look ahead from.
...
function restoreSpaceBefore (node, selector) {
	...
	if (/** @type {NodeWithRaws} */ (node).raws?.spaces?.before === undefined) node.spaces.before = trailingWhitespace(selector, node.sourceIndex)
}
...
function setSpaceBefore (target, value) {
	...
	let spaces = /** @type {NodeWithRaws} */ (target).raws?.spaces
