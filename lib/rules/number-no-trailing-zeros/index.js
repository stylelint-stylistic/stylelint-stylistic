	...
	return (root, result) => {
		...
		function check (node, value) {
			...
			valueParser(value).walk((valueNode) => {
				...
				let match = (/\.(\d{0,100}?)(0+)(?:\D|$)/u).exec(valueNode.value)
				...
				// our startIndex is identical to our index except when we have only trailing zeros after our decimal. in that case we don't need the decimal either so we move our index back by 1.
