...
testRule({
	ruleName,
	config: [`always`],
	extraRules: { "@stylistic/function-comma-newline-after": `always` },
...
// The rule takes the breaks out of a nested call in the same pass, so the twin's lineness is asked of the outer call as all of them leave it (#704)
testRule({
	ruleName,
	config: [`never`],
	extraRules: { "@stylistic/function-comma-newline-after": `always-multi-line` },

	reject: [
		{
			description: `a break behind the comma of a nested call, the only break of the outer call, which both commas lose, leaving a call the twin passes over`,
			code: `a { b: f(g(1,\n2), 3) }`,
			fixed: `a { b: f(g(1,2),3) }`,
			warnings: [
				{
