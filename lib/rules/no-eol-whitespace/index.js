...
import { EVERY_CR_OR_LF, TRAILING_SPACES_AND_TABS } from "../../regexps.js"
...
function findErrorStartIndex (lastEOLIndex, string, { ignoreEmptyLines, isRootFirst }) {
	...
	if (ignoreEmptyLines) {
		// If there is only whitespace between the previous newline and this newline, ignore
...
function rule (primary, secondaryOptions) {
	return (root, result) => {
		...
		function eachEolWhitespace (string, callback, { isRootFirst = false, isPlainText = false } = {}) {
			...
			if (isPlainText) {
				for (let { index } of string.matchAll(EVERY_CR_OR_LF)) handleEol(index)
