 ...
 * @param {FunctionNode} node - The media feature being fixed.
 * @param {string} text - The whitespace to put there.
 * @param {string} params - The parameters the node's positions are counted in.
 * @returns {Edit} The edit that writes it.
 */
function closingEdit (node, text, params) {
					...
					if (primary === `never`) {
						...
						if (SPACE_OR_TAB.test(node.after)) {
							...
							problems.push({
								...
								fix: isFixable ? () => { addEdit(edits, closingEdit(node, ``, params)) } : undefined,
							})
						}
					}
