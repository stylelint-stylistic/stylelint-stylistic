...
import { MEDIA_AT_RULE, SPACE_OR_TAB } from "../../regexps.js"
 ...
 * @param {FunctionNode} node - The media feature being fixed.
 * @param {string} text - The whitespace to put there.
 * @returns {Edit} The edit that writes it.
 */
function openingEdit (node, text) {
 ...
 *
 * A feature the file leaves unclosed carries no parenthesis to stand in front of: the parser hands out no whitespace of its own for one, and the stringifier prints what a fix puts there on the very end of the parameters. The span is named the same way here, so that such a write stays where it has always been written — a query the parser has read this way is #131's, not this rule's.
 * @param {FunctionNode} node - The media feature being fixed.
 * @param {string} text - The whitespace to put there.
 * @returns {Edit} The edit that writes it.
 */
function closingEdit (node, text) {
	let end = node.unclosed ? node.sourceEndIndex : node.sourceEndIndex - 1
...
function rule (primary) {
	return (root, result) => {
		...
		root.walkAtRules(MEDIA_AT_RULE, (atRule) => {
			...
			let spellsInlineComments = readsInlineComments(atRule, result)
			...
			let inlineComments = findInlineCommentSpans(params, endsInlineCommentOnFormFeed(atRule), spellsInlineComments)
				...
				if (node.type === `function`) {
					...
					if (primary === `never`) {
						...
						if (SPACE_OR_TAB.test(node.after)) {
							...
							let isFixable = !endsWithInlineComment(params.slice(0, node.sourceEndIndex - 1 - node.after.length), spellsInlineComments)

							problems.push({
								message: messages.rejectedClosing,
								index: node.sourceIndex - 2 + len + indexBoost,
								fix: isFixable ? () => { addEdit(edits, closingEdit(node, ``)) } : undefined,
							})
						}
					}
						...
						if (node.after === ``) {
							problems.push({
								...
								index: node.sourceIndex - 2 + len + indexBoost,
								fix () { addEdit(edits, closingEdit(node, ` `)) },
							})
						}
					}
				}
