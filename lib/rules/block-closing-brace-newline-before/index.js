...
function rule (primary, _secondaryOptions, context) {
	return (root, result) => {
		...
		function check (statement) {
			...
			let after = (statement.raws.after || ``).replace(/;+/u, ``)
			...
			// We're really just checking whether a newline *starts* the block's final space -- between the last declaration and the closing brace. We can ignore any other whitespace between them, because that will be checked by the indentation rule.
			if (!after.startsWith(`\n`) && !after.startsWith(`\r\n`)) {
				...
				report({
					...
					fix () {
						...
						if (primary.startsWith(`always`)) {
							let firstWhitespaceIndex = statementRaws.after.search(/\s/u)
						...
						else if (primary === `never-multi-line`) statementRaws.after = statementRaws.after.replaceAll(/\s/gu, ``)
					},
				})
			}
		}
