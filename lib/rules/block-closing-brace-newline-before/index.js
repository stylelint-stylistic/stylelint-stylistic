...
import { EVERY_WHITESPACE, LF_OR_CRLF, SEMICOLON_RUN, WHITESPACE } from "../../regexps.js"
...
function rule (primary, _secondaryOptions, context) {
	return (root, result) => {
		...
		function check (statement) {
			...
			let after = (statement.raws.after || ``).replace(SEMICOLON_RUN, ``)
			...
			// What is checked is whether a newline *starts* the block's final space — the run between the last declaration and the closing brace. The rest of that whitespace is the indentation rule's business.
			if (!after.startsWith(`\n`) && !after.startsWith(`\r\n`)) {
				...
				report({
					...
					fix () {
						...
						if (primary.startsWith(`always`)) {
							let firstWhitespaceIndex = statementRaws.after.search(WHITESPACE)
							...
							let newlineIndex = newlineAfter.search(LF_OR_CRLF)
						...
						else if (primary === `never-multi-line`) statementRaws.after = statementRaws.after.replaceAll(EVERY_WHITESPACE, ``)
					},
				})
			}
		}
