...
import { SEMICOLON_RUN } from "../../regexps.js"
...
function rule (primary, secondaryOptions, context) {
	return (root, result) => {
		...
		function check (statement) {
			...
			let before = (statement.raws.after || ``).replace(SEMICOLON_RUN, ``)
			...
			report({
				...
				fix () {
					...
					// The break to write where no whitespace of the statement's own tree holds one, and the only place this rule needs it: an empty line is otherwise spelled with a break the file already uses
