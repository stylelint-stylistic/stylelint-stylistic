...
testRule({
	...
	reject: [
		...
		{
			description: `a selector broken with a line feed over a block broken with carriage returns, the block's own break being the one written`,
			code: `a,\nb {\rcolor: pink; }`,
			fixed: `a,\nb {\rcolor: pink; \r\r}`,
			line: 2,
			column: 18,
		...
		{
			description: `a block whose own children open no line, the break of the block nested in it being the one written`,
		...
		{
			description: `a block broken only between a declaration's colon and its value, which is whitespace of the block like any other`,
			code: `a {color:\rpink;}`,
		...
		{
			description: `a block broken only behind an at-rule's name, which the whitespace of the tree holds all the same`,
