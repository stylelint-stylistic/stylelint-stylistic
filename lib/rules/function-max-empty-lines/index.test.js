...
testRule({
	...
	reject: [
		...
		{
			description: `a call standing behind a comment a form feed closes, which Sass reads a line in`,
			code: `a { b: f(1) // c\fg(\n\n\n2)\n; }`,
			fixed: `a { b: f(1) // c\fg(\n2)\n; }`,
			line: 1,
			column: 17,
...
testRule({
	...
	accept: [
		...
		{
			description: `the same comment closed by a form feed, which Less reads no line in, so the call behind it is the comment's text as well`,
			code: `a { b: f(1) // c\fg(\n\n\n2)\n; }`,
		},
	],
