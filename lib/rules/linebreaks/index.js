...
import { isAtRule, isDeclaration, isRule } from "../../utils/typeGuards/index.js"
...
function rule (primary) {
	return (root, result) => {
		...
		/**
		 * Rewrites the line breaks of the texts a node holds: its selector, its value, its at-rule parameters, the text of a comment, and the whitespace in front of the node and at the end of its block. The raws it leaves standing — what an at-rule writes between its name and its parameters, what any node writes between its two halves, the raw of a `!important`, and the whitespace inside the delimiters of a comment — are #283.
		 *
		...
		function fix () {
			root.walk((node) => {
				...
				if (isAtRule(node)) setAtRuleParams(node, fixData(getAtRuleParams(node)))
