...
import { findMediaOperator } from "../../utils/findMediaOperator/index.js"
		...
		root.walkAtRules(/^media$/iu, (atRule) => {
			...
			if (fixOperatorIndices.length > 0) {
				...
				for (let index of fixOperatorIndices.toSorted((a, b) => b - a)) {
					...
					if (primary === `always`) params = beforeOperator + afterOperator.replace(/^\s*/u, ` `)
					else if (primary === `never`) params = beforeOperator + afterOperator.replace(/^\s*/u, ``)
				}
