...
testRule({
	...
	reject: [
		...
		{
			// A bare address behind the operator: `>=url` is one word to the tokenizer and the parentheses are code, while behind `>= url` they are one token closed at the first parenthesis, which a string holds here
			description: `a bare address glued to the operator whose parentheses hold a string with a closing parenthesis, which is left unfixed`,
			code: `@media (width>=url(a ")" b)) { a { b: c; } }`,
			fixed: `@media (width>=url(a ")" b)) { a { b: c; } }`,
			line: 1,
			column: 16,
			message: messages.expectedAfter(),
		},
		...
		{
			description: `a run of two spaces between the operator and a bare address, which the fix shortens without changing the word in front of the name`,
			code: `@media (width>=  url(a.png)) { a { b: c; } }`,
			fixed: `@media (width>= url(a.png)) { a { b: c; } }`,
			line: 1,
			column: 16,
			message: messages.expectedAfter(),
		},
		{
			description: `a bare address glued to the operator whose parentheses hold nothing the two readings part on, which is spaced out`,
			code: `@media (width>=url(a.png)) { a { b: c; } }`,
			fixed: `@media (width>= url(a.png)) { a { b: c; } }`,
			line: 1,
			column: 16,
...
testRule({
	...
	reject: [
		...
		{
			// The other way round: behind `>= url` the parentheses are one token closed at the first parenthesis, and glued to the operator they are code reading a string nothing closes
			description: `a space between the operator and a bare address whose parentheses hold a quotation mark, which is left unfixed`,
			code: `@media (width>= url(a"b)) { a { b: c; } }`,
			fixed: `@media (width>= url(a"b)) { a { b: c; } }`,
			line: 1,
			column: 16,
		...
		{
			description: `the same address behind the operator of a bare query, left unfixed too`,
			code: `@media a= url(a"b) { a { b: c; } }`,
			fixed: `@media a= url(a"b) { a { b: c; } }`,
			line: 1,
			column: 10,
		...
		{
			description: `a space between the operator and a bare address whose parentheses hold nothing the two readings part on, whose space goes`,
			code: `@media (width>= url(a.png)) { a { b: c; } }`,
			fixed: `@media (width>=url(a.png)) { a { b: c; } }`,
			line: 1,
			column: 16,
