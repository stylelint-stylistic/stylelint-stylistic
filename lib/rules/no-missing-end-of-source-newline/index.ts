	...
	return (root, result) => {
		...
		report({
			...
			fix () {
				...
				//
				// The run comes off only where the raw and the file agree it ends the file. The raw is the only place to write: behind a `//` comment ending the file a bare carriage return or form feed is that comment's `raws.right` to `postcss-less`, and the root's raw is empty there, while `postcss-scss` ends the comment on it and files it in the raw. The file is what the warning was about: `no-extra-semicolons` listed ahead has already taken the semicolon out of the raw, which then ends on a break while the file ends on the semicolon. Asked of both, the fix ends the same way on either side of those rules; beside `linebreaks` the order still decides.
