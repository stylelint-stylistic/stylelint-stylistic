...
function rule (primary) {
	return (root, result) => {
		...
		function check (node, checkedValue, getIndex) {
			/** @type {Array<{ index: number, endIndex: number, message: import('stylelint').RuleMessage, messageArgs: string[] }>} */
			let problems = []
			 ...
			 * @returns {{ index: number, endIndex: number, message: import('stylelint').RuleMessage, messageArgs: string[] } | null} What to report about the unit, or `null` where the node carries no miscased one.
			 */
