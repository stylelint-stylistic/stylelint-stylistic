...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `an inline comment opened inside the function standing in front, closed by the very line break this option would take away: the fix would leave the first argument, and everything the declaration has behind it, inside the comment, so the value is left alone and the warning stands`,
			code: `
				a {
					t: foo(1px // c) calc(
					2px);
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `the same comment with a space where that line break stood: the argument is inside the comment before the fix as after it, so the fix goes through`,
			code: `a { t: foo(1px // c) calc( d); }`,
			fixed: `a { t: foo(1px // c) calc(d); }`,
			line: 1,
			column: 27,
			message: messages.rejectedOpening,
		},
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `an inline comment closed by the line break in front of the first argument, under the option that would write a single space in its place: a space closes the comment no better than nothing does, so the value is left alone and the warning stands`,
			code: `
				a {
					t: foo( 1px // c ) calc(
					2px );
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `the same value under the syntax that rewrites an inline comment of a value into a block comment, which leaves the spelling of the file in a copy of its own: that copy is the one the guard reads, and the value is left alone`,
			code: `
				a {
					t: foo(1px // c) calc(
					2px);
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `a form feed in place of that line break, which Sass ends a comment on and Less does not: the reading counting one has the argument outside the comment as the value stands and inside it once the fix has run, so the value is left alone`,
			code: `a { t: foo(1px // c) calc(\fd); }`,
			fixed: `a { t: foo(1px // c) calc(\fd); }`,
			line: 1,
			column: 27,
			message: messages.rejectedOpening,
		},
