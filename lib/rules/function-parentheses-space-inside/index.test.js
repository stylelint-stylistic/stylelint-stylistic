...
testRule({
	...
	accept: [
		...
		},
		{
			...
			description: `a form feed with nothing behind it that closes the comment: Less reads no line in one, so the comment runs to the end of the value and the parenthesis the parser closed the call on stands in its text`,
...
testRule({
	...
	accept: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/132
			description: `the same separator in front of a form feed, Less reading no line in either character`,
			code: `a { transform: translate( 1px, 2px //c\u2028\f); }`,
		},
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/320
			description: `a form feed inside the comment, which Less reads no line in: the comment runs to the end of the value and the parenthesis the parser closed the call on stands in its text`,
			code: `a { b: f(1px // c\f2px); }`,
		},
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/320
			description: `a closing parenthesis written on the line an inline comment runs to, which is the parenthesis the parser closed the call on`,
...
testRule({
	...
	accept: [
		...
		},
		{
			...
			description: `a double slash with no whitespace in front of it, which postcss-scss leaves in the value rather than recording as a comment: the plugin therefore reads no form feed as the end of that comment and the parenthesis stands in its text, though Sass itself ends the comment there and would have the whitespace in front of the parenthesis reported`,
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `the same comment with a space where that line break stood, which closes nothing: the argument stands outside the comment Sass ended on the form feed before the fix as after it, so the space goes`,
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `a separator of Unicode standing where the first argument opens, which the value parser counts as no whitespace and the fix leaves standing: the break in front of it goes and the separator stays, so the argument opens on it`,
			code: `a { t: 1px // c\fcalc(\n\u2028x); }`,
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/280
			description: `a form feed closing the comment in front of the call, and a line break where the first argument opens: Sass ends the comment on that form feed, so the argument stands outside it before the fix as after, and the single space the option asks for goes in`,
			code: `a { t: 1px // c\fcalc(\n2px ); }`,
			fixed: `a { t: 1px // c\fcalc( 2px ); }`,
			line: 1,
			column: 22,
