...
function printedBefore (printedValue, declValue, index) {
	...
	// The syntax rewrote nothing, so the two copies are one and the same text, and reading them
	// as two would rewrite a form feed into a line feed where Less reads no line in one at all
...
function beforeClosingString (decl, declValue, valueNode, spellsInlineComments) {
	...
	// The fix writes the whitespace the function keeps in front of the parenthesis and nothing else,
	// so everything behind that whitespace stays on the line it is written on, and only the parenthesis moves
	...
	// writes in place of that whitespace, so such a value is left to the fix as it always was.
	// The parenthesis is written back on the end for both askings. `endsWithInlineComment` reads
	// trailing whitespace as room a fix is about to write in, which is the question its other callers
	// ask and the opposite of this one, and the value parser counts as space only what stands below
	// the blank, so a separator of Unicode survives the fix as a word — trimmed away with the line
	// break it holds, it would leave the comment in front of it read as still open.
	// The two askings read a line break differently as well. Taking a comment for one that already
	// holds the parenthesis lets a breaking fix through, while taking one for still open at the end
	// only holds a safe fix back, so the first counts every break anything reads as one and the
	// second only the breaks both syntaxes do.
	if (endsWithInlineComment(`${standing.replaceAll(closingBreak, `\n`)})`, spellsInlineComments)) return ``

	// A single space is all the `always` options put there, and a space closes no comment, so both
	// options leave the parenthesis standing behind the same text
	return `${kept})`
}
	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			parsedValue.walk((valueNode) => {
				 ...
				 *
				 * The parenthesis goes right after the text this reads, and the whitespace the fix
				 * overwrites ends it. Where an inline comment stands there, the line break that
				 * whitespace holds is what closes the comment, so no option can be satisfied without
				 * taking the parenthesis, and everything the declaration has left, into the comment's
				 * text: leave the value alone and let the warning stand. The single-line options ask
				 * as well — `isSingleLineString` reads a line feed and a carriage return and nothing
				 * else, so a function whose only break is a form feed passes for a single line while
				 * Sass ends an inline comment on it all the same, and such a function meets a comment
				 * the options were held to be out of reach of.
				 *
				 * Only one option is ever in force, so this is asked once at most, and only where a
				 * problem has been found: reading the whole value in front of the parenthesis is not
				 * work to do for a function nothing is the matter with.
				...
				function isClosingFixable () {
					...
					// spells none: holding a fix back over a comment that is not there refuses a write
