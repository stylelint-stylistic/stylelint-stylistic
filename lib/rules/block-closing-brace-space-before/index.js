...
function rule (primary) {
	...
	return (root, result) => {
		...
		function check (statement) {
			...
			let isFixable = !writesIntoInlineComment(statement.last, result, after.replace(TRAILING_WHITESPACE, ``))
					...
					report({
						...
						fix: isFixable
							? () => {
								...
								if (primary.startsWith(`always`)) statementRaws.after = statementRaws.after.replace(TRAILING_WHITESPACE, ` `)
								else if (primary.startsWith(`never`)) statementRaws.after = statementRaws.after.replace(TRAILING_WHITESPACE, ``)
							}
							: undefined,
					})
				},
