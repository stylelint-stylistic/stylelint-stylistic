				...
				err: (msg) => {
					report({
						...
						fix () {
							...
							if (primary.startsWith(`always`)) statementRaws.after = statementRaws.after.replace(/\s*$/u, ` `)
							else if (primary.startsWith(`never`)) statementRaws.after = statementRaws.after.replace(/\s*$/u, ``)
						},
					})
				},
			})
