...
import { hideFalseInlineComments } from "../../utils/hideFalseInlineComments/index.js"
...
function rule (primary) {
	return (root, result) => {
		...
		/**
		 * Builds the copy of a text `style-search` is handed in place of the text itself.
		 *
		 * The search reads the comments of the text for itself, and by rules of its own: an inline
		 * comment ends on a line feed and on nothing else, a double slash opens one wherever it stands —
		 * the one in `myurl(//a)` among them, which a file of plain CSS spells no comment with — and the
		 * `*\/` closing one block comment and the `/*` opening the next are read as the two slashes of a
		 * third. The copy has every comment blanked out of it and every false opening spelled out of
		 * harm's way, so that none of the three readings is left to make. It is as long as the text and
		 * spells it character for character everywhere else, so every position stands where it did, and
		 * the checks that follow are made against the text as it stands.
		 * @param {string} text - The text the search is to run over.
		 * @param {import('postcss').Node} node - The node it was read from, which the syntax is asked about.
		 * @returns {string} The copy to hand the search.
		 */
		function searchCopy (text, node) {
			let commentSpans = findCommentSpans(text, endsInlineCommentOnFormFeed(node), readsInlineComments(node, result))
			...
			return hideFalseInlineComments(blankComments(text, commentSpans), [])
		}

		root.walkAtRules(/^import$/iu, (atRule) => {
			let param = getAtRuleParams(atRule)
			let searchString = searchCopy(param, atRule)
		...
		root.walkDecls((decl) => {
			...
			let searchString = searchCopy(value, decl)
