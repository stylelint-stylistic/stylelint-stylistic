...
import { closeInlineComments } from "../../utils/closeInlineComments/index.js"
...
function rule (primary) {
	return (root, result) => {
		 ...
		 * @param {string} searchString - The same value, with the break closing each of its inline comments written as a line feed, which is the only one `style-search` ends such a comment with.
		...
		root.walkAtRules(/^import$/iu, (atRule) => {
			...
			let searchString = closeInlineComments(param, findInlineCommentSpans(param, endsInlineCommentOnFormFeed(atRule)))
		...
		root.walkDecls((decl) => {
			...
			let searchString = closeInlineComments(value, findInlineCommentSpans(value, endsInlineCommentOnFormFeed(decl)))
