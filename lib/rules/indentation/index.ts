...
function checkNodeLine (scope: IndentationScope, node: ChildNode, nodeLevel: number): void {
	...
	// The raw where the parser filed one, and otherwise the run PostCSS prints in front of a node a rule of another plugin built without one, which is what the file will hold
	...
	// The indentation is the whitespace opening the last line of `before`, a form feed or bare carriage return as much as a space; the writers below read the same run. What stands behind it is on the line, not in front of it: a `*` or `_` hack, a stray semicolon, or a styled template's interpolation, whose own breaks end no line of the stylesheet
	let beforeSpans = syntax.hostCodeSpans(before, node)
	let beforeBreaks = lastLineStart(before, beforeSpans) >= 0

	// A first node with no break in front stands on the stylesheet's opening line and is asked to be empty, not for the host line's tabs. A bare carriage return ends a JavaScript line and none of the stylesheet's, so a node behind one still stands there
...
function rule (ruleScope: RuleScope<typeof MESSAGES>, primary: PrimaryOption, secondaryOptions: SecondaryOptions = {}): RuleCheck {
	...
	return (root, result) => {
		...
		root.walk((node) => {
			...
			// The line a statement's semicolon opens closes the statement, so it stands at the statement's own level, as a closing brace stands at its block's
			...
			// A line a free semicolon opens with nothing behind it is an empty statement of its container; last, since the node's and the brace's fixes write such a line at their own level
...
// Reads every line a run's writers touch, so it takes the run's last turn
