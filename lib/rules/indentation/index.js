...
import { CRLF, EVERY_BARE_CR_OR_FORM_FEED, EVERY_LINE_BREAK, EVERY_LINE_BREAK_AND_INDENT, EVERY_LINE_INDENT, EVERY_LINE_INDENT_WITH_CONTENT, EVERY_LINE_SPACE_INDENT, EVERY_SPACE, EVERY_TAB, FIRST_LINE, INDENT_AT_END, LEADING_CLOSING_BRACE, LEADING_CLOSING_PARENTHESIS, LEADING_INDENT_AND_CONTENT, LEADING_SPACES_AND_TABS, LINE_BREAK, OPENING_BRACE_AT_END, OPENING_PARENTHESIS_AT_END, OPENS_WITH_TAG, SPACES_AND_TABS_BEFORE_CONTENT, TRAILING_LINE_BREAK, TRAILING_STAR_OR_UNDERSCORE } from "../../regexps.js"
...
function rule (primary, secondaryOptions = {}) {
	return (root, result) => {
		...
		function checkMultilineBit (source, newlineIndentLevel, node, nodeLevel) {
			...
			// The search is given one target and finds every break of the text all the same, since the copy spells each of them with a line feed: a bare carriage return and a form feed are written over one character at a time, so the copy stays as long as the text and the index of a match is the index of the last character of the break in the file. A Windows pair is left as it stands, its line feed being what the target finds.
			searchString = searchString.replaceAll(EVERY_BARE_CR_OR_FORM_FEED, `\n`)
