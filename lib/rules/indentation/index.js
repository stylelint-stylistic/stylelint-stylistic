...
function rule (primary, secondaryOptions = {}) {
	return (root, result) => {
		...
		function indentationLevel (node, level = 0) {
			...
			if (isRoot(node.parent)) return level + getRootBaseIndentLevel(node.parent, baseIndentLevel, primary)
			...
			if (optionsMatches(secondaryOptions, `except`, `block`) && (isRule(node) || isAtRule(node)) && hasBlock(node)) calculatedLevel--
			...
			styleSearch(
				...
				(match, matchCount) => {
					...
					if (!ignoreInsideParans && match.insideParens) {
						...
						if (source[match.startIndex - 1] === `\r`) newlineIndex--
...
function inferRootIndentLevel (root, baseIndentLevel, indentSize) {
	...
	if (indents.length) return Math.max(...indents.map((indent) => getIndentLevel(indent))) + newBaseIndentLevel
