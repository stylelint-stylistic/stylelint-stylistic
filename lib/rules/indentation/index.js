...
function rule (primary, secondaryOptions = {}) {
	return (root, result) => {
		...
		function checkMultilineBit (source, newlineIndentLevel, node) {
			...
			let firstLineParenthesesArePaidFor = !isRule(node) || (`params` in node && Boolean(node.params))
			...
			styleSearch(
				...
				(match, matchCount) => {
					...
					if (!ignoreInsideParens && match.insideParens) {
						...
						let followsOpeningParenthesis = OPENING_PARENTHESIS_AT_END.test(source.slice(0, newlineIndex))
