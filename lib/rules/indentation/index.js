...
import { isAtRule, isDeclaration, isRoot, isRule } from '../../utils/typeGuards'
import { isBoolean, isNumber, isString, assertString } from '../../utils/validateTypes'
...
function inferRootIndentLevel (root, baseIndentLevel, indentSize) {
	...
	if (!isNumber(baseIndentLevel) || !Number.isSafeInteger(baseIndentLevel)) {
		...
		const indentions = source.match(/^[ \t]*(?=\S)/gm)
