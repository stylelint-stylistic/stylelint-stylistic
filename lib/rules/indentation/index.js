...
function rule (primary, secondaryOptions = {}) {
	return (root, result) => {
		...
		function checkSelector (ruleNode, ruleLevel) {
			// The lines are measured in the copy the file spells, since that is the text the
			// positions of a warning are counted in and the text a fix is written to. The text of
			// each inline comment is blanked out of the scan's copy: `style-search` would read the
			// line break that closes one as part of it and never hand the position over, and the
			// lines behind such a comment would go unmeasured. The blanks keep every position
			// where it was, and the fix is written to the text as the file spells it. One thing
			// stays unmeasured: a line holding nothing but an inline comment is all blanks to the
			// scan, which anchors each line's indentation on the first character that is not
			// whitespace and finds none there.
			let fileSelector = ruleSelector(ruleNode)
			let selector = blankInlineComments(fileSelector, findInlineCommentSpans(fileSelector, true))
			let level = ruleLevel
		...
		function checkMultilineBit (source, newlineIndentLevel, node) {
			...
			// A value or a set of params is measured one level above its node, so the parentheses
			// the first line opens are already paid for; a selector is measured at the node's own
			// level, and a pseudo-class opening its parentheses at the end of a line raises the
			// lines inside them — and only them, the other selectors of the list standing outside
 ...
 * `raws.selector.raw`, keeps the spelling of the file in `raws.selector.scss` and prints that
 * second copy, while `rule.selector` is a copy with every comment taken out. This rule measures
 * lines and writes lines, so the text the file holds is the one it has to work on: it is the text
 * the positions of a warning are counted in, and the only text a fix can reach.
 ...
 *
 * Where `postcss-scss` keeps two copies, the second is the one that is printed, so the fix goes
 * there, and the raw is refilled beside it the way the syntax fills it, for the rules that come
 * after. Assigning `node.selector` would have PostCSS throw both raws away, and the comments of
 * the selector with them.
 * @param {import('postcss').Rule} node - The rule node.
