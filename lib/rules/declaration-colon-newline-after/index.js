	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			for (let i = 0, l = propPlusColon.length; i < l; i += 1) {
				...
				let indexToCheck = (/^[^\S\r\n]*\/\*/u).test(propPlusColon.slice(i + 1)) ? propPlusColon.indexOf(`*/`, i) + 1 : i
					...
					err: (m) => {
						report({
							...
							fix () {
								...
								decl.raws.between = (/^\s*\n/u).test(betweenAfter) ? betweenBefore + betweenAfter.replace(/^[^\S\r\n]*/u, ``) : betweenBefore + context.newline + betweenAfter
							},
						})
					},
				})
