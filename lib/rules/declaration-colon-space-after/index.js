		...
		declarationColonSpaceChecker({
			...
			fix: (decl, index) => {
				...
				if (primary.startsWith(`always`)) {
					if (isSpaceInValue) decl.raws.value.raw = ` ${decl.raws.value.raw.replace(/^\s*/u, ``)}`
					else decl.raws.between = between.slice(0, colonIndex) + between.slice(colonIndex).replace(/^:\s*/u, `: `)
				...
				if (primary === `never`) {
					...
					// Remove only the leading whitespace of the raw value.
					if (isCustomProperty(decl.prop) && decl.raws.value) {
						decl.raws.value.raw = decl.raws.value.raw.replace(/^\s+/u, ``)
					}
					else {
