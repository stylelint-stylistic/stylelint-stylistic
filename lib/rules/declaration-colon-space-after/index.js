	...
	return (root, result) => {
		...
		declarationColonSpaceChecker({
			...
			fix: (decl, index) => {
				...
				if (isSpaceInValue) decl.raws.value.raw = `${space}${decl.raws.value.raw.replace(/^\s*/u, ``)}`
				else decl.raws.between = between.slice(0, colonIndex) + between.slice(colonIndex).replace(/^:\s*/u, `:${space}`)
