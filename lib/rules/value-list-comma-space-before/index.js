...
function rule (primary) {
	...
	return (root, result) => {
		...
		valueListCommaWhitespaceChecker({
			...
			// rule could write would reach it.
			...
			isFixable: (declNode, index, declString) => index >= declarationValueIndex(declNode) && !endsWithInlineComment(declString.slice(0, index), readsInlineComments(declNode, result)),
			fix: (declNode, index) => {
		...
		if (fixData) {
			for (let [decl, commaIndices] of fixData.entries()) {
				// The commas are written from the back of the declaration forward, so that a fix never
				// moves the text a later one is counted in. The comma opening the value is the one that
				// moves `declarationValueIndex`, and being the first of them it is written last.
				for (let index of commaIndices.toSorted((a, b) => b - a)) {
					let valueIndex = index - declarationValueIndex(decl)

					// The whitespace in front of a comma opening the value is none of the value's: it is
					// the text standing between the colon and the value, which `raws.between` holds, and
					// no write to the value could reach it. It is also the text `declarationValueIndex`
					// counts, so the positions already reported are counted in it as it was.
					if (valueIndex === 0) {
						let between = decl.raws.between || `:`

						decl.raws.between = primary.startsWith(`always`) ? between.replace(/\s*$/u, ` `) : between.replace(/\s*$/u, ``)
