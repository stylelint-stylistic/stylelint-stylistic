...
function rule (primary) {
	...
	function fixBefore (attributeNode) {
		...
		if (primary === `always`) setAttrBefore(attrBefore.replace(/^\s*/u, ` `))
		else if (primary === `never`) setAttrBefore(attrBefore.replace(/^\s*/u, ``))
	}
	...
	function fixAfter (attributeNode) {
		...
		if (primary === `always`) setAfter(after.replace(/\s*$/u, ` `))
		else if (primary === `never`) setAfter(after.replace(/\s*$/u, ``))
	}
}
