...
Under `--fix` this rule breaks the blocks it can before any rule with a `-single-line` or `-multi-line` option reads them, and reports only after those rules have written, so it reports the blocks the fixed file holds on one line.
