...
testRule({
	...
	reject: [
		...
		},
		{
			...
			description: `a block broken by a form feed, which makes it multi-line like any other break`,
...
testRule({
	...
	reject: [
		...
		{
			// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/248
			description: `an inline comment abutting the brace and closed by a bare carriage return, which ends its line as readily as a line feed`,
			code: `a {// c\rcolor: pink;\r}`,
			fixed: `a {// c\rcolor: pink;\r}`,
			line: 1,
			column: 4,
			message: messages.rejectedAfterMultiLine(),
		},
 ...
 * @returns {Promise<{ code: string, warnings: number }>} What the fix wrote and how many warnings a run without it raised.
 */
...
describe(`${ruleName} on the whitespace it carries past a comment`, () => {
	...
	it(`carries nothing onto a comment that stands behind a break of its own`, async () => {
