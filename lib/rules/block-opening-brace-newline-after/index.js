 ...
 * @param {import('postcss').Node} nodeToCheck - The first node of the block that is not a comment, which the run ends in front of.
 ...
 * @type {import('stylelint').RuleBase<'always' | 'always-multi-line' | 'never-multi-line', { ignore?: 'rules' | 'rules'[] }>}
 */
function rule (primary, secondaryOptions) {
	...
	return (root, result) => {
		...
		function check (statement) {
			 ...
			 * @param {import('postcss').Node} comment - The comment the walk stepped over.
			 * @param {import('postcss').Node | undefined} nextNode - The node standing behind that comment.
			 */
			function carryBreakPastComment (comment, nextNode) {
				if (!nextNode) return
