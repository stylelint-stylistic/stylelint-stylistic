...
describe(`a styled template under the core's rules`, () => {
	it(`is refused with one warning naming the styled namespace, however many rules are configured — attributed to the first rule that takes its turn, which \`indentation\` no longer is, its check taking the run's last turn`, async () => {
	...
	it(`is read by the same rules under the namespace the warning names, \`indentation\`'s warning arriving last with its turn`, async () => {
