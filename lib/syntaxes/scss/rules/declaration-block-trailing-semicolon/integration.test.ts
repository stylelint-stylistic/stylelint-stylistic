...
// The neighbor is named and this rule listed behind it, as in the core's integration test; these blocks add that the settings read are the namespace's own, and that a `//` comment closing the block is a node of its own here, which the semicolon is written in front of.
