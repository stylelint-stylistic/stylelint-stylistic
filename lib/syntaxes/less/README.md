...
- `unit-case` ends a unit in front of the first escape, since Less reads what an escape spells as a value of its own — `10px\#fff` compiles to `10px \#fff`, and a fix recases the `px` alone — where the core reads the escape as a character of the unit;
