...
- `unit-case` ends a unit in front of the first escape or hyphen, since Less reads a unit as ASCII letters and underscores and what stands behind either as a value of its own — `10px\#fff` compiles to `10px \#fff` and `10PX-A` to `10PX A`, and a fix recases the unit alone — where the core reads both as characters of the unit; what a hyphen parts off is read again, so `10PX-2REM`, which Less subtracts, has the two units `PX` and `REM`;
