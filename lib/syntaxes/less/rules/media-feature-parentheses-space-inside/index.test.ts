...
testRule({
	...
	accept: [
		...
		{
			// Sass answers alike for both spellings of the name here, as in a declaration's value: `@media (min-width: aurl(a/b))` and `@media (min-width: éurl(a/b))` both compile, while `aurl(a//b)` and `éurl(a//b)` in their place both fail at the length of the file, the `//` comment having carried off the parenthesis.
