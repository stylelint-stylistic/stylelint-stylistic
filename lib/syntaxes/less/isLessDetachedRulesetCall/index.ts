...
/**
 * Asks whether an at-rule has the shape of a call to a Less detached ruleset, `@dr()`, whatever its name.
 *
 ...
 * @returns True where the node has that shape.
 */
export function hasLessDetachedRulesetCallShape (atRule: AtRule | LessAtRule): boolean {
...
export function isLessDetachedRulesetCall (atRule: AtRule | LessAtRule): boolean {
	return hasLessDetachedRulesetCallShape(atRule) && LESS_DETACHED_RULESET_NAME.test(atRule.name)
}
