...
import { closingSemicolonMayBeCommentText } from "../closingSemicolonIsCommentText/index.ts"

/**
 * Asks whether the `//` comment a semicolon of its text may have closed a node in runs on right behind a node.
 *
 * It does behind that node, whatever declaration or bodiless at-rule it is, since behind a variable, `@extend` or another at-rule which reading Less takes turns on the value and on what follows (1790151466), and behind a comment `postcss-less` carved out of its text: one whose whole `raws.before` is the comment's and whose text holds no break. Behind a carved `//` comment the break closing both opens the raw behind, so the head there is empty.
...
function runsOnBehind (node: ChildNode, result: PostcssResult): boolean {
	if (!isComment(node)) return closingSemicolonMayBeCommentText(node, result)
...
/**
 * Reads the head of a node's `raws.before`, or of a block's `raws.after`, that may be the text of a `//` comment behind a node the parser closed on a semicolon of that text ([#720](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/720)), for a writer to keep off.
 *
