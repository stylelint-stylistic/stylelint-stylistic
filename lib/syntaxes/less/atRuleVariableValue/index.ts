...
export function atRuleVariableValue (atRule: AtRule): AtRuleVariableValue | null {
	...
	return {
		...
		write (fixed: string, edits: Edit[]): void {
			// Where the head ends in the fixed text, read off the edits so that nothing here assumes a fixed text keeps the length of the one read. No edit spans the raw between the two, which is whitespace and comments
