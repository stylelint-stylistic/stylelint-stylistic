...
export function isStandardSyntaxDeclaration (decl) {
	...
	// the value is the extend call. A property `&` is nothing CSS has, whatever the value —
	// the compiler reads an extend there and answers anything else, `&:EXTEND(.b)` and
	// `& :extend(.b)` among it, with a syntax error — so the shape alone is the answer. The
	// `extend` mark the syntax puts beside the node goes unasked: it is matched against the
	// text of any value at all, quotes and all, so `b: "extend(x)"` and `b: myextend(y)`
	// carried it too, though both are plain declarations Less compiles as they stand.
