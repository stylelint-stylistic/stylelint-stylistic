 ...
 *
 * PostCSS's tokenizer, which `postcss-less` reads by and `postcss-scss` copies with patches of its own, takes parentheses as one `brackets` token where nothing from the `(` to the first `)` is a line break, a quotation mark, a `(`, a solidus or a backslash; the token is opaque to the parser, while code is read for groups, and a `[` nothing closes before the `)`, or a `{` in a custom property's value or an at-rule's params, is a group the parser finds open at the end of the declaration and refuses the file over, or at the end of the params, where the at-rule gets no block and they run to the end of the file. A break written into the token switches the reading. A group a `]` behind that `)` closes counts as open, though a stray `)` then balances the call. Not modelled, since such parentheses holding an open group are a file the parser refused, or an at-rule's params it swallowed the file into, before any rule ran: PostCSS reads a `(` standing in front of the first `)` behind an earlier `(` it read as code the same way whatever it holds, which `postcss-scss` does not; the fix refused over such params changes nothing. Nor is the `#{` `postcss-scss` reads as an interpolation in code and refuses unclosed, since Sass refuses such a file itself.
