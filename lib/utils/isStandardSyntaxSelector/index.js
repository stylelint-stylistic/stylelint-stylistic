...
export function isStandardSyntaxSelector (selector) {
	...
	// as readily as the spaced form. The word is read in lower case only, as Less reads its
	// keywords: `.a:hover WHEN (1 = 1)` is printed by the compiler as it stands, and
	// `when NOT (1 = 1)` is a syntax error to it.
	if ((/\swhen\s*(?:not\s*)?\(/u).test(code)) return false
