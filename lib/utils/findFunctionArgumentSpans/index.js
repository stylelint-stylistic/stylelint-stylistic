import { IDENTIFIER_CODE_POINT, LEADING_HEX_ESCAPE, LINE_BREAK, OPENS_NO_IDENTIFIER } from "../../regexps.js"
 ...
 * @returns {number | null} The index behind what was skipped, or null where code stands there.
 */
 ...
 *
 * An escape spells one character of an identifier with several, and CSS spells it two ways: a backslash and up to six hexadecimal digits, closed by one whitespace character, and a backslash and any single character besides. So `\66 oo` is `foo` and `\)` is a parenthesis that closes nothing. A backslash escapes nothing where a line break stands behind it or where the text ends there — it is a delimiter of its own then, and no character of any name.
 * @param {string} text - The text being scanned.
 * @param {number} index - The index of the backslash.
 * @returns {number} The number of characters the escape occupies, or zero where the backslash opens none.
 */
function escapeLength (text, index) {
	let next = text[index + 1]

	if (next === undefined || LINE_BREAK.test(next)) return 0

	let hexadecimal = text.slice(index).match(LEADING_HEX_ESCAPE)

	return hexadecimal ? hexadecimal[0].length : 2
}
 ...
 * @param {string} text - The text being scanned.
 * @param {number | null} runStart - The index the run of identifier code points in front of the parenthesis opens at, or null where the character in front of it is no such code point.
 * @param {number} runEnd - The index the last run of the text closed at, or minus one where it has held none.
 * @param {number} index - The index of the opening parenthesis.
 * @returns {string} The name, or the empty string where the run is no identifier and no operator names the call.
 */
function readName (text, runStart, runEnd, index) {
