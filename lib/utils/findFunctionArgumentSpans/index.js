 ...
 *
 * A name is what `TRAILING_FUNCTION_NAME` reads, and several things that stand in front of a parenthesis spell none. An interpolation spells none: `#{$a}(1,2)` and `@{v}(1,2)` name nothing that can be looked up, and `#{$q}(min-width: 1px)` is a media feature behind an interpolated query. Neither does a run of name characters that opens on a digit, `2and(` and `2px(` being a dimension in front of a parenthesis. Nor does one that closes on a hyphen, `a-(`, nor one written outside ASCII, nor one an escape spells, `fo\6f(`, nor one opening on `--` and a digit, nor Less's `%` — those five are names where the pattern reads none, and #252 carries them. Where the two readings cannot be told apart from the text, taking such a parenthesis for a group is the reading that loses nothing here — a check is made where perhaps none was needed, rather than dropped where one was.
 *
