 ...
 *
 * Both texts are asked of each reading the syntax leaves owed, and the answer is yes where one and the same reading has the character outside a comment as the file stands and inside one once the fix has run. A reading holding it inside on both sides describes a value already broken, which the fix leaves no worse; a reading holding it outside on both sides describes one the fix does not reach. Putting one question to one reading and the other to the other — which is what the closing guard of `function-parentheses-space-inside` did until #132 — answers about two languages at once and declines fixes neither language is at risk from.
 *
...
export function movesEndIntoInlineComment (standingText, fixedText, reading) {
	return formFeedReadingsOwed(reading).some((endsOnFormFeed) => {
		/** @type {import('../readsInlineComments/index.js').InlineCommentReading} */
		let one = { ...reading, endsOnFormFeed }

		return !endsWithInlineComment(standingText, one) && endsWithInlineComment(fixedText, one)
	})
}
