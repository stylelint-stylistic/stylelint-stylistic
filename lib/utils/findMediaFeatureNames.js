...
export default function findMediaFeatureNames (mediaQueryParams, callback) {
	...
	const mediaQueryConditions = list.flatMap((listItem) => listItem.flatMap((componentValue) => {
		if (
		...
		mediaQuery.walk(({ node }) => {
			...
			if (isGeneralEnclosed(node)) {
				topLevelTokenNodes(node).forEach((token, i, topLevelTokens) => {
					...
					if (
						/* Media Feature */ (!prevToken && nextToken && nextToken[0] === TokenType.Colon) || /* Range Feature */ (nextToken && nextToken[0] === TokenType.Delim && rangeFeatureOperator.test(nextToken[4].value)) || /* Range Feature */ (prevToken && prevToken[0] === TokenType.Delim && rangeFeatureOperator.test(prevToken[4].value))
					) {
						callback(token)
					}
				})
