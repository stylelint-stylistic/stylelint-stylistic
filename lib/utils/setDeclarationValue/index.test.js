...
describe(`setDeclarationValue`, () => {
	...
	it(`writes the copy the syntax prints`, () => {
		...
		expect(/** @type {import('../typeGuards/index.ts').SyntaxRaw} */ (node.raws.value).scss).toBe(`0 // c\n  2px`)
	})
	...
	it(`keeps the raw beside it in step`, () => {
		...
		expect(/** @type {import('../typeGuards/index.ts').SyntaxRaw} */ (node.raws.value).raw).toBe(`0 /* c*/\n  2px`)
	})
