...
export function mediaFeatureColonSpaceChecker (opts) {
	opts.root.walkAtRules(/^media$/iu, (atRule) => {
		...
		// `style-search` ends an inline comment with a line feed and with nothing else, and opens one on
		// a double slash wherever it stands, the one in `url(http://x/y.png)` among them. The break
		// closing each real comment is written as a line feed for the search, and the false openings are
		// spelled out of harm's way, so that neither a comment nor an address hides the features behind
		// it; every position stands where it did, and the callers are handed the parameters as they are
		// spelled.
