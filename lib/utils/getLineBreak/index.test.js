...
import { getLineBreak } from "./index.js"
 ...
 * @param {Record<string, unknown>} [rules] - The rules the configuration lists.
 * @returns {string} The break.
 */
function ask (css, rules = {}) {
	let root = parse(css)

	return getLineBreak(/** @type {import('postcss').Rule} */ (root.first), result(rules))
}

/**
 * Builds the least of a Stylelint result that holds a configuration.
 * @param {Record<string, unknown>} [rules] - The rules the configuration lists.
 * @returns {import('stylelint').PostcssResult} The result.
 */
function result (rules = {}) {
	return /** @type {import('stylelint').PostcssResult} */ (/** @type {unknown} */ ({ stylelint: { config: { rules } } }))
}
