 ...
 * @returns {string | undefined} The break the file ends its lines with, or `undefined` where nothing can be read: a file written on one line, and a node standing in no file at all, which is one made by hand.
 */
function lineBreakOfFile (node) {
 ...
 * @param {import('postcss').Node} node - A node of the file the break is written into.
 * @param {import('stylelint').PostcssResult} result - The Stylelint result, which holds the configuration.
 * @returns {string} The break to write.
 */
export function getLineBreak (node, result) {
	let setting = result.stylelint?.config?.rules?.[LINEBREAKS_RULE]
	let option = Array.isArray(setting) ? setting[0] : setting

	if (typeof option === `string` && option in BREAK_OF_OPTION) return BREAK_OF_OPTION[/** @type {keyof typeof BREAK_OF_OPTION} */ (option)]
