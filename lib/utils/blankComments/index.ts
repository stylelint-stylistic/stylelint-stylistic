 ...
 * @param spans - The comment spans, where already known from whichever scan reads the text; only these are blanked.
 * @returns The text with every comment replaced by spaces.
 */
export function blankComments (text: string, spans: { start: number, end: number }[] = findCommentSpans(text)): string {
