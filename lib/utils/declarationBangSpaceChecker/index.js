 ...
 * @typedef {{ start: number, text: string, write: (text: string) => void, edits: Edit[] }} DeclarationPart
 */
 ...
 * @param {DeclarationPart[]} parts - The writable texts, in the order they are printed; the edits are added to them in place.
 * @param {Edit} edit - The edit, measured in the printed declaration.
 * @returns {void}
 */
function fileEdit (parts, edit) {
			...
			opts.locationChecker({
				...
				err: (message) => {
					report({
						...
						fix: isFixable
							? () => {
								for (let edit of opts.fix({ text: declString, index })) fileEdit(parts, edit)
							}
							: undefined,
					})
				},
