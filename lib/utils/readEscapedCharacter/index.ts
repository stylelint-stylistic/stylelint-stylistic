 ...
 *
 * Only a comment's delimiter is read that way here, where the tokenizer refuses every solidus: a block comment's always, and a `//` comment's where the syntax's own tokenizer reads one, which is `postcss-scss` ([#517](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/517)); Less and Sass read that escape, and `postcss-less` keeps it in the value. Reading a lone `\/` the tokenizer's way would make `\/url(` an address, where the word standing in front of the parenthesis is `/url` and no address opens for anyone.
