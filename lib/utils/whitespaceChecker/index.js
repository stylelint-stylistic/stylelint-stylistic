...
/**
 * Tells whether a character ends a line. A line feed, a carriage return and a form feed all do —
 * PostCSS's tokenizer switches on the three, `postcss-scss` spells them out as one class, and CSS's
 * own preprocessing folds `\r` and `\f` into `\n` before anything else — so a checker asking for a
 * newline has to take any of them, or a file broken with one of the other two is reported for a
 * break it already holds and, where the fix writes a `\n` in front of that break, grows by a line
 * with every run.
