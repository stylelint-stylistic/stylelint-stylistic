...
import { NAME_END_IN_A_WORD } from "../../regexps.ts"
...
/**
 * Reads the name a `(` opens its parentheses under out of the word the value parser hands back, which holds whatever stands in front of that name with no whitespace between: `[c]url` is one word to it and a square bracket, an identifier and an address to every tokenizer. A bracket an escape covers ends nothing, since `@csstools/css-tokenizer` reads it as a character of the name and Less and Sass refuse such a text; PostCSS's own tokenizer parts from them there and ends the word on the backslash.
...
function readNameInWord (name: string): string {
	...
	while (index < name.length) {
		...
		if (NAME_END_IN_A_WORD.test(name.charAt(index))) opening = index + 1
