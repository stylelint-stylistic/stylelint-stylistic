...
import { inlineCommentReading, syntaxKeepsInlineComments, syntaxSpellsInlineComments } from "./index.js"
 ...
 * @param {unknown} [syntax] - The syntax the file was opened with, none of it standing for plain CSS.
 * @returns {import('./index.js').InlineCommentReading} What that syntax makes of such a comment.
 */
function read (syntax) {
	let rule = /** @type {import('postcss').Rule} */ (parse(`a { color: red; }`, { from: undefined }).first)

	return inlineCommentReading(/** @type {import('postcss').Declaration} */ (rule.first), /** @type {import('stylelint').PostcssResult} */ (/** @type {unknown} */ ({ opts: { syntax } })))
}
