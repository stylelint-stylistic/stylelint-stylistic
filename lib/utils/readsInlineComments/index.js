...
/** @typedef {{ spells: boolean, keeps: boolean }} InlineCommentReading What the probe of a syntax says about a comment opened by a double slash: whether the syntax spells one at all, and whether it leaves one standing in the value a rule reads. Which break closes one is no question of the syntax's: a line break is what PostCSS reads as one, a line feed with or without a carriage return in front of it, and every scan of a text closes such a comment there. */
...
function probeSyntax (syntax) {
	...
	let reading = { spells: true, keeps: false }
