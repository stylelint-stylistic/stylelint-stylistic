 ...
 *
 * The two spellings are identical, so the text cannot answer this and the syntax has to: a double
 * slash of plain CSS is code — part of an address, most often, `myurl(//a)` or `a//b` — and reading
 * it as a comment silences everything behind it on the line. Answering costs one parse of a probe,
 * and the answer is kept against the syntax it was given by — which is once per file rather than
 * once per run, since Stylelint builds a fresh syntax for every file unless the syntax was named to
 * it as a string.
 *
 * A stylesheet embedded in a page carries the syntax of its own block, and it is that one the
 * question belongs to: the syntax the file was opened with parses the page rather than the style,
 * and one page may hold blocks written in several languages.
...
function probeSyntax (syntax) {
	...
	// A syntax that makes nothing of the probe has said nothing, and the answer that costs nothing is
	// the one that reads a comment as a comment
	...
	try {
		...
		// The raw is the copy a syntax rewrites the comments of a value in, and the second question is
		// asked only where that copy is the one the rule reads, so a double slash surviving there is
		// what it turns on — not one the syntax has left standing in a copy beside it
		...
		// A syntax that cannot parse the probe has said nothing about it, and the verdict standing is
		// the one that reads a comment as a comment
	}
