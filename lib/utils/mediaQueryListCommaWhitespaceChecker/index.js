...
export function mediaQueryListCommaWhitespaceChecker (opts) {
	opts.root.walkAtRules(/^media$/iu, (atRule) => {
		...
		// A comma standing inside the arguments of a function is a comma of those arguments and of no
		// list: the one in `url(x/a,b.png)` names the file as surely as the letters around it do, and
		// whitespace written beside it would name another file. `valueListCommaWhitespaceChecker` has
		// asked the search itself to pass such a comma over since it was written; the search cannot be
		// asked here, since it reads the parenthesis a set of media parameters opens on as the opening
		// of a call and would pass over the whole of the first query.
