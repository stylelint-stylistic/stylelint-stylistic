...
export default function report (problem) {
	const { ruleName, result, message, messageArgs, line, node, index, endIndex, word, severity } = problem
	...
	const ruleSeverity = typeof ruleSeverityOption === `function` ? ruleSeverityOption(...(messageArgs || [])) || defaultSeverity : ruleSeverityOption
...
function printfLike (format, ...args) {
	return args.reduce((/** @type {string} */ result, arg) => result.replace(/%[ds]/, String(arg)), format)
}
