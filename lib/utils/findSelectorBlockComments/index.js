/**
 * Finds the block comments of a selector, passing over what stands inside a quoted attribute
 * value, since an attribute value may hold any text at all — `[x="/*"]` opens no comment.
 *
 * An escape is read before a quotation mark is, so that a quote escaped outside a string —
 * the one in `.x\'y`, which Less takes for a class of that name — opens nothing.
