 ...
 * @returns {string | undefined} The break the file ends its lines with, or `undefined` where nothing can be read: a file written on one line, a file whose only breaks stand inside its comments and its strings, and a node standing in no file at all, which is one made by hand, alone or in a tree made the same way — a node taken out of a parsed tree keeps the file it was read from and answers about it still.
 */
