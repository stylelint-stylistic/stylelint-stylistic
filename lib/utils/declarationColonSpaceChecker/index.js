...
export function declarationColonSpaceChecker (opts) {
	opts.root.walkDecls((decl) => {
		...
		if (isCustomProperty(decl.prop) && decl.value !== `` && (/^\s+$/u).test(decl.value)) {
			// For custom properties with comments, PostCSS stores whitespace after the colon in raws.value.
			// Only its leading whitespace belongs to the colon.
			const valuePrefix = decl.raws.value ? decl.raws.value.raw.match(/^\s*/u)?.[0] || `` : decl.value
