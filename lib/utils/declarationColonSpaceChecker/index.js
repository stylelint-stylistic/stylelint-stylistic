...
export function declarationColonSpaceChecker (opts) {
	opts.root.walkDecls((decl) => {
		...
		if (isCustomProperty(decl.prop) && decl.value !== `` && (/^\s+$/u).test(decl.value)) {
			// For custom properties whose value holds a comment, PostCSS stores that value in `decl.raws.value`, and the whitespace after the colon goes there with it.
			// Only the leading whitespace of the raw value belongs to the colon.
			let valuePrefix = decl.raws.value ? getDeclarationValue(decl).match(/^\s*/u)[0] : decl.value

			// The slice reaches to where the value starts, so that the whitespace taken from the value is added once and not twice.
			// `endOfPropIndex` reaches one character further for every character standing in front of the colon, and those characters come out of the value itself.
