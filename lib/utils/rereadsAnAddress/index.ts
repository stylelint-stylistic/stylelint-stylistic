import { AT_WORD_AT_END, LEADING_CSS_WHITESPACE, OPENS_WITH_QUOTE_OR_CSS_WHITESPACE, TOKENIZER_TOKEN_END } from "../../regexps.ts"
...
/**
 * Asks whether the word the tokenizer reads in front of a `url` standing behind a text is `url` itself. A comment's closing delimiter ends the word in front of it, and so does an escaped character, which the tokenizer reads as a word of its own; `postcss-scss`'s tokenizer ends a word on a comma too, but not an at-word.
 ...
 */
function joinsTheName (text: string, reading: Pick<CommentReading, `tokenizes`>): boolean {
	let last = text.at(-1)
