 ...
 *
 * `postcss-less` keeps a `//` comment behind a declaration's value or a bodiless at-rule's params in that text, and `postcss-scss` keeps one behind an at-rule's params in `raws.between`, so the semicolon the block's flag prints behind the text would land inside the comment ([#232](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/232)). In front of the comment it closes the code, which is where Less wants it: a mixin call or a declaration compiles the same with it and without, and an `@extend` compiles only with it ([#423](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/423)). A block comment in front of the `//` comment stays with the code, in front of the semicolon, where the compiler prints it; one behind the code with no `//` comment behind it is no such run, since the semicolon has stood behind block comments all along.
 *
