...
describe(`isStandardSyntaxDeclaration`, () => {
	...
	it(`nested scss list`, () => {
		equal(isStandardSyntaxDeclaration(scssDecl(`a { $list: (key: value, key2: value2) }`)),
			false,
		)
	})
	...
	it(`scss map declaration with quotes`, () => {
		equal(isStandardSyntaxDeclaration(scssDecl(`$foo: ("key": value, "key2": value2)`)),
			false,
		)
	})
	...
	it(`scss complex value in parentheses`, () => {
		equal(isStandardSyntaxDeclaration(scssDecl(`a { height: (3px + 5px) * (2px + 4px) }`)),
			true,
		)
	})
