...
describe(`isStandardSyntaxCombinator`, () => {
	it(`tag`, () => {
		...
		expect(isStandardSyntaxCombinator(/** @type {import('postcss-selector-parser').Combinator} */ (/** @type {unknown} */ (parse(`a {}`).first)))).toBe(false)
	})
	it(`descendant`, () => {
 ...
 * @returns {import('postcss-selector-parser').Combinator} That combinator.
 */
function combinator (css) {
	/** @type {import('postcss-selector-parser').Combinator[]} */
	let list = []
