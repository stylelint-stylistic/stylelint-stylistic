...
import { findInterpolationSpans, findInterpolationSpanTouching } from "./index.js"

/**
 * Builds the node a value parse would hand a rule for a run of a text.
 * @param {string} text - The text the value was parsed from.
 * @param {string} run - The run of it the node stands on, taken at its first occurrence.
 * @param {string} [type] - What the parser calls the node.
 * @returns {{ type: string, value: string, sourceIndex: number, sourceEndIndex: number }} The node.
 */
function node (text, run, type = `word`) {
