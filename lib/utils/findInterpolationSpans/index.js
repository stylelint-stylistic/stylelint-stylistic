import { EVERY_INTERPOLATION } from "../../regexps.js"

/**
 * The span an interpolation occupies in a text, counted in that text.
 * @typedef {{ start: number, end: number }} InterpolationSpan
 */
 ...
 * @returns {InterpolationSpan[]} The spans, in the coordinates of the scanned text.
 */
export function findInterpolationSpans (text) {
 ...
 * @param {Pick<import('postcss-value-parser').Node, 'sourceIndex' | 'sourceEndIndex'> & { type?: string, value?: string }} valueNode - The node the walk has reached, of which only its span is read.
 * @param {InterpolationSpan[]} spans - The spans {@link findInterpolationSpans} found in the text the node was parsed from, which the node's positions count in.
 * @returns {InterpolationSpan | undefined} The span the node touches, or nothing where the node carries no interpolation's text.
 */
export function findInterpolationSpanTouching (valueNode, spans) {
