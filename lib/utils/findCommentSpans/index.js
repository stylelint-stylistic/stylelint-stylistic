...
/**
 * Finds the spans the comments of a text occupy in it, block comments and inline ones alike. A
 * double slash belonging to an address opens no comment, whether the address is quoted or bare
 * inside `url()`; a block comment runs to its `*\/` and an inline one to the end of its line.
 *
 * The span of a block comment holds its delimiters, since the two of them are as much the comment as
 * its text is. The span of an inline comment ends where the break that closes it begins, the break
 * itself staying outside: a caller taking the comment out of the text keeps the line it stood on.
 * A syntax that spells no comment with a double slash — plain CSS is one — has none to find, and
 * the caller says so. The pair is then two characters of code like any other, and the second of them
 * opens a block comment where a `*` follows it: `//*c*\/` holds one comment in such a file and none
 * of the other kind, which is the reading that keeps `1px //*c,d*\/,2px` in one piece.
...
export function findCommentSpans (text, endsOnFormFeed = false, spellsInlineComments = true) {
	...
	while (index < text.length) {
		...
		if (character === `"` || character === `'`) {
			...
			// whether the text it holds was spelled by a syntax that reads one. Where nothing is said,
			// the comment is taken for the longer of the two, which only ever hides code from a rule,
			// while taking it for the shorter hands the rule the text of a comment to work on.
