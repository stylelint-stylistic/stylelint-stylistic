import { WORD_BRACE_OR_HYPHEN } from "../../regexps.js"
...
/**
 * Skips the name of a `url()` and the parenthesis it opens on, whose three letters CSS lets a stylesheet spell with an escape wherever it likes: `\75 rl(`, `u\rl(` and `URL(` open the token that `url(` opens, and Sass and every browser read the address of all four. Every other place in the plugin that recognises an address matches the four characters instead, which is [#344](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/344).
 * @param {string} text - The string being scanned.
...
function skipUrlName (text, openIndex) {
	...
	return text[index] === `(` ? index + 1 : openIndex
}
 ...
 * @param {boolean} behindIdentifier - True where the character run in front of the index is one this scan reads as part of a name, which is a word character, a closing brace or a hyphen, or an escape spelling anything at all. A name of code points outside ASCII is none to this question and is none to it on either side of #321, {@link IDENTIFIER_CODE_POINT} being the wider reading `findFunctionArgumentSpans` puts to the same text; that is [#343](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/343).
...
export function findCommentSpans (text, endsOnFormFeed = false, spellsInlineComments = true) {
	...
	while (index < text.length) {
		...
		if (character === `\\`) {
			...
			behindIdentifier = WORD_BRACE_OR_HYPHEN.test(character)
			index += 1
		}
	}
