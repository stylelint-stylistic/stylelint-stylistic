...
export function getDimension (syntax: Syntax, node?: Partial<Node>): {
	...
	// Less ends a unit at the first escape whatever it spells (`10P\9X` is `10P` and the keyword `\9X`), at the first hyphen (`10PX-A` is `10PX` and the keyword `A`) and at the first digit (`10PX9` is `10PX` and the number `9`), so no hack comes out and the cut below falls on whichever of the three stands first
	let readsIdentifier = syntax.readsUnitAsIdentifier()

	// The hacks come off wherever they stand, only where the file spells an escape (`10PX\\0` ends in an escaped backslash); the escape's closing whitespace stays, so the unit of `10px\9 2PX` ends at the space
