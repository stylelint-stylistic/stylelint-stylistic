 ...
 * @param {Partial<import('postcss-value-parser').Node>} [node] - The value parser node, or nothing at all.
 * @returns {{unit: null, number: null, positions: null} | (valueParser.Dimension & { positions: number[] })} The dimension object, or null values where the node carries no dimension.
 */
export function getDimension (node) {
	 ...
	 * @param {number} start - Where the run opens in the copy.
	 * @param {number} length - How many characters the run holds.
	 * @returns {void}
	 */
	function take (start, length) {
