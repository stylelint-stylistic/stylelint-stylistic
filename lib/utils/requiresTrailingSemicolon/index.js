 ...
 * - a call to a detached ruleset, `@dr()`, which takes no arguments and no space in front of its parentheses. A space makes it an at-rule again, and Less asks for the semicolon behind `@dr ()` as it asks for it behind `@import`.
 *
...
function isLessAtRule (atRule) {
	...
	return !(atRule.params === `()` && atRule.raws.afterName === ``)
}
