 ...
 * @param {any} syntax - The syntax the stylesheet is parsed with, and the one the question is asked under unless another is named.
 * @param {string} code - The stylesheet.
 * @param {any} [asked] - The syntax to ask the question under, where it is not the one that parsed.
...
function lastNodeOfTheBlock (syntax, code, asked) {
	...
	return requiresTrailingSemicolon(node, { opts: { syntax: asked === undefined ? syntax : asked } })
}
...
describe(`requiresTrailingSemicolon`, () => {
	describe(`the syntax`, () => {
		it(`no syntax at all, which is plain CSS`, () => {
			expect(requiresTrailingSemicolon(parse(`a { @layer l; }`).first.first, { opts: {} })).toBe(false)
		})
		...
		it(`the syntax of the node's own stylesheet, which is asked ahead of the one the file was opened with`, () => {
			...
			expect(requiresTrailingSemicolon(root.first.last, { opts: { syntax: less } })).toBe(false)
		})
