 ...
 * @param {{ parse: import('postcss').Parser }} syntax - The syntax the stylesheet is parsed with, and the one the question is asked under unless another is named.
 * @param {string} code - The stylesheet.
 * @param {unknown} [asked] - The syntax to ask the question under, where it is not the one that parsed.
 * @returns {boolean} What the util makes of that node.
 */
function lastNodeOfTheBlock (syntax, code, asked) {
	let root = syntax.parse(code, { from: undefined })
	let block = /** @type {import('postcss').Container} */ (root.last)
	let node = block.nodes ? block.last : block

	if (!node) throw new Error(`The block holds no node`)

	return requiresTrailingSemicolon(node, /** @type {import('stylelint').PostcssResult} */ (/** @type {unknown} */ ({ opts: { syntax: asked === undefined ? syntax : asked } })))
}

/**
 * The same, with the node named on its own and the block spelled around it.
 * @param {string} node - The text of the node closing the block.
 * @returns {boolean} What the util makes of that node under Less.
 */
function closingALessBlock (node) {
...
describe(`requiresTrailingSemicolon`, () => {
	describe(`the syntax`, () => {
		it(`no syntax at all, which is plain CSS`, () => {
			let rule = /** @type {import('postcss').Rule} */ (parse(`a { @layer l; }`).first)

			expect(requiresTrailingSemicolon(/** @type {import('postcss').AtRule} */ (rule.first), /** @type {import('stylelint').PostcssResult} */ (/** @type {unknown} */ ({ opts: {} })))).toBe(false)
		})
		...
		it(`the syntax of the node's own stylesheet, which is asked ahead of the one the file was opened with`, () => {
			...
			let source = /** @type {import('../typeGuards/index.ts').EmbeddedSource} */ (root.source)
			let rule = /** @type {import('postcss').Rule} */ (root.first)

			source.syntax = scss

			expect(requiresTrailingSemicolon(/** @type {import('postcss').AtRule} */ (rule.last), /** @type {import('stylelint').PostcssResult} */ (/** @type {unknown} */ ({ opts: { syntax: less } })))).toBe(false)
		})
		...
		it(`one parse of the probe per syntax, however many nodes are asked about`, () => {
			...
			let counted = {
				 ...
				 * @param {string} code - The stylesheet.
				 * @param {import('postcss').ProcessOptions} [options] - The options of the parse.
				 * @returns {import('postcss').Root | import('postcss').Document} What Less makes of it.
				 */
				parse (code, options) {
					parses += 1
