 ...
 *
 * Behind an at-rule with neither block nor semicolon (`@extend .b`, a Less mixin call) PostCSS puts the run into the at-rule's `raws.between` and leaves `raws.after` empty. Not where a semicolon or, under `postcss-less`, a mixin call's `!important` is printed behind that raw: the `less` namespace moves the run behind the flag into `raws.after` wherever it finds the flag in the file, so an empty one means no run stands there. Behind a custom property with no semicolon the parser hands the tokenizer nothing back, so the run stays in the value, or in `raws.important` behind a flag. A filled `raws.after` was written by an earlier fix of the pass; a missing one is refused, since PostCSS would compute a default.
