 ...
 *
 * PostCSS moves the comments and whitespace behind the params into `raws.between`, but a `//` comment is a word to its tokenizer, so `postcss-less` leaves one standing behind a mixin call or a variable written without a semicolon in the params; measured with them, its line was asked for the params' level, a level deeper than the block it is a line of, which is how Less reads the comment. So the head ends where the code does, and what stands behind it opens the swallowed lines. The cut runs over `raws.afterName` as well, since a call written without parentheses, `.m⏎// c`, has the comment for its whole params and the break in front of them; and it is made only where the at-rule carries no block, since in front of an opening brace the comment's line is a line of the params, as it is under every parser filing it into `raws.between`.
 *
 * With neither block nor semicolon an at-rule runs to its block's closing brace, and PostCSS puts everything in between into `raws.between`. Such a line is the block's; the tree is read as it stands, so a neighbor's semicolon moves the comment at once, and the trailing whitespace is the run in front of the brace, `getBlockAfter`'s. Behind a Less mixin call's flag those lines are in `raws.important`, printed behind `raws.between`; the flag's line is blanked, since it is measured behind a semicolon neither. Behind a stylesheet's last at-rule the parser files them into the root's `raws.after`, printed behind both.
