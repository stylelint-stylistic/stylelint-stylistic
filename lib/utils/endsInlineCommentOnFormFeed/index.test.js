...
describe(`endsInlineCommentOnFormFeed`, () => {
	...
	it(`a selector this syntax keeps a copy of its own of`, () => {
		expect(endsInlineCommentOnFormFeed(scssRule(`a, // c\n  b {}`))).toBe(true)
	})
	...
	it(`a selector of a syntax that keeps its comments where they stand`, () => {
		expect(endsInlineCommentOnFormFeed(lessRule(`a, // c\n  b {}`))).toBe(false)
	})
...
function lessRule (css) {
	...
	less.parse(css).walkRules((rule) => list.push(rule))
...
function scssRule (css) {
	...
	parseScss(css).walkRules((rule) => list.push(rule))
