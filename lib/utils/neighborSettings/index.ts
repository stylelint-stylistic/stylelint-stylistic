...
/** A neighboring rule: its directory name and the primaries it accepts. */
export type NeighborRule = {
	name: string,
	options: string[],
}
...
/** A neighbor's copy as the configuration lists it: the caller's key, the option, the setting whole, the configured name and the syntax of its namespace. */
 ...
 * @param result - The PostCSS result carrying the configuration.
 * @param rules - The neighbors by the caller's keys; a key may stand empty.
 * @returns Key, option, whether the fix is off and the configured name, per neighbor, in run order.
 */
export function neighborSettings<Key extends string> (node: Node, result: PostcssResult, rules: Partial<Record<Key, NeighborRule>>): [Key, string, boolean, string][] {
	return listedInRunOrder(node, result, rules).map(({ key, option, setting, name }) => [key, option, fixDisabledBy(setting), name])
}

/** A neighbor as a configured name registers it: the caller's key, the rule and the syntax of its namespace. */
type NeighborName<Key extends string> = { key: Key, rule: { name: string, options: (string | true)[] }, syntax: Syntax }

/** The names each table of neighbors is configured under, built once per table. */
let namesByTable: WeakMap<object, Map<string, NeighborName<string>>> = new WeakMap()

/**
 * Maps every name a table's neighbors can be configured under, one per namespace, to what it registers.
 * @param rules - The neighbors by the caller's keys.
 * @returns The map.
 */
function namesOf<Key extends string> (rules: Partial<Record<Key, { name: string, options: (string | true)[] }>>): Map<string, NeighborName<Key>> {
	let names = namesByTable.get(rules) as Map<string, NeighborName<Key>> | undefined

	if (names) return names

	let made: Map<string, NeighborName<Key>> = new Map()
...
/** A neighboring rule whose primary is a keyword or `true`, read with its secondaries. */
export type NeighborRuleSetting = {
	name: string,
	options: (string | true)[],
}

/** One copy of a neighbor, read whole: its option, whether its fix is off, its secondaries, the name it is configured under and the syntax of that name's namespace. */
export type NeighborCopy = {
	option: string | true,
	fixDisabled: boolean,
...
/** The one-key table each neighbor is looked up through, so the names it is configured under are built once. */
 ...
 * @returns The copy, in a list of one; none where the neighbor is unlisted, refuses its primary, or is listed only under namespaces refusing the root.
 */
export function neighborCopies (node: Node, result: PostcssResult, rule: NeighborRuleSetting): NeighborCopy[] {
