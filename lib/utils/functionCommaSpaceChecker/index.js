...
export function functionCommaSpaceChecker (opts) {
	opts.root.walkDecls((decl) => {
		...
		parsedValue.walk((valueNode) => {
			...
			// The reading that takes the comments out of the arguments is the reading that placed the
			// commas: one scan of the run, whose spans say where every comment stands and which double
			// slash opens one. The false openings are spelled out of harm's way in the same copy, so that
			// nothing downstream of it reads an address for a comment either.
			let commentSpans = findCommentSpans(argumentsRun, endsOnFormFeed)
			let hiddenArguments = hideFalseInlineComments(argumentsRun, commentSpans.filter(({ isInline }) => isInline))
			...
			// A comment standing behind code takes only itself out; one followed by nothing but whitespace
			// takes the whitespace in front of it too
