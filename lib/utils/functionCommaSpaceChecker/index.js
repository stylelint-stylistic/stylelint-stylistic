...
export function functionCommaSpaceChecker (opts) {
	opts.root.walkDecls((decl) => {
		...
		let valueCommentSpans = findCommentSpans(declValue, endsOnFormFeed, reading.spells)
		...
		parsedValue.walk((valueNode, at, siblings) => {
			...
			let commentSpans = findCommentSpans(argumentsRun, endsOnFormFeed, reading.spells)
				...
				return (message) => {
					...
					report({
						...
						fix: opts.fix && isFixable(commaNode)
							? () => {
								edits.push(...opts.fix(commaNode, nodeIndex, valueNode))
							}
							: undefined,
					})
				}
