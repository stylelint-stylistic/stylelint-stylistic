 ...
 *   fix: ((node: ValueParserDivNode, index: number, nodes: ValueParserNode[]) => Edit[]),
...
export function functionCommaSpaceChecker (opts) {
	opts.root.walkDecls((decl) => {
		...
		let valueCommentSpans = findCommentSpans(declValue, endsOnFormFeed, readsComments)
				...
				return (message) => {
					...
					report({
						...
						fix: opts.fix && isFixable(commaNode)
							? () => {
								edits.push(...opts.fix(commaNode, nodeIndex, valueNode.nodes))
							}
							: undefined,
					})
				}
