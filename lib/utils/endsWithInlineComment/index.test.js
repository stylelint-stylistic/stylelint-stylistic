...
describe(`endsWithInlineComment`, () => {
	...
	it(`inline comment a form feed leaves open, since Less reads no line in it`, () => {
	...
	it(`a syntax spelling no comment with a double slash ends in code, whatever the text holds`, () => {
		expect(endsWithInlineComment(` // keep me`, false)).toBe(false)
		expect(endsWithInlineComment(`1px//c`, false)).toBe(false)
		expect(endsWithInlineComment(`myurl(//a)`, false)).toBe(false)
	})
