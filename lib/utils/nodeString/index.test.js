...
import { nodeString } from "./index.js"
...
describe(`nodeString`, () => {
	...
	it(`falls back to PostCSS's own stringifier where no syntax is to be had`, () => {
		...
		expect(nodeString(root.first)).toBe(`a { color: pink }`)
	})
})

/**
 * Parses a stylesheet and prints the first node of the type asked for.
 * @param {any} syntax - The syntax to read the stylesheet with, and to print the node back with.
...
function first (syntax, css, type) {
	...
	return nodeString(found, { opts: { syntax } })
}
