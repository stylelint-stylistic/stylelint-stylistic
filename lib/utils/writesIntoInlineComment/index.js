 ...
 */
export function writesIntoInlineComment (node, result) {
	// The syntax read this comment as an inline one itself, so there is nothing here to scan: whatever is written behind it is its text, until the line ends
	if (isComment(node)) return Boolean(node.inline || node.raws.inline)

	return endsWithInlineComment(textAWriteFollows(node), readsInlineComments(node, result))
}
