...
function ask (syntax, code, pick, spelledBetween) {
	...
	return writesIntoInlineComment(pick(root), { opts: { syntax } }, spelledBetween)
}
...
describe(`writesIntoInlineComment`, () => {
	...
	it(`a form feed standing in the middle of the value, which Sass ends the comment on`, () => {
		expect(ask(scss, `a {\n\tcolor: red // c\f2px\n\t;\n}`, (root) => root.first.first)).toBe(false)
	})

	// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/333
	it(`the same form feed under a syntax that normalises the line endings of a file and reads no line in one`, () => {
		expect(ask(less, `a {\n\tcolor: red // c\f2px\n\t;\n}`, (root) => root.first.first)).toBe(true)
	})

	// https://github.com/stylelint-stylistic/stylelint-stylistic/issues/333
	it(`a form feed the value ends with, which is the very whitespace the write goes into: the break that closed the comment is what the fix takes away, whichever language reads a line in it`, () => {
		expect(ask(scss, `a {\n\tcolor: red // c\f;\n\ttop: 0;\n}`, (root) => root.first.first)).toBe(true)
		expect(ask(less, `a {\n\tcolor: red // c\f;\n\ttop: 0;\n}`, (root) => root.first.first)).toBe(true)
	})
	...
	it(`an inline comment of its own, which nothing a caller spells behind it closes`, () => {
		expect(ask(scss, `a {\n\tcolor: red;\n\t// c\n\ttop: 0;\n}`, (root) => root.first.first.next(), `;`)).toBe(true)
	})
