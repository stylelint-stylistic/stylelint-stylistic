...
/**
 * Empties every quoted run of a selector and takes out every comment, keeping the length of what stood there, so that neither an attribute value nor a comment can be read as selector syntax.
 *
 ...
 *
 * The `s` flag is what lets a backslash escape a line break, as a string spanning more than one line does; without it such a string would not be recognised at all, and its text would go on being read as selector syntax.
...
export function withoutQuotedTextAndComments (selector) {
	return selector.replaceAll(EVERY_ESCAPE_STRING_OR_COMMENT, (match) => {
