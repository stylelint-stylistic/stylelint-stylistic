...
import { moveDeclarationValueHeadIntoBetween } from "./index.js"

/**
 * Reads the first declaration of a stylesheet, moves the head of its value, and says what came of it.
 * @param {import('postcss').Parser} parser - The parser to read the stylesheet with.
 ...
 * @returns {{ printed: string, value: string, between: string }} What the file prints, what the value now reads as, and what stands between the property and it.
 */
function move (parser, css, length) {
	let root = parser.parse(css)
	let decl = root.first.first

	moveDeclarationValueHeadIntoBetween(decl, length)

	return { printed: root.toResult({ syntax: parser }).css, value: getDeclarationValue(decl), between: decl.raws.between }
}
