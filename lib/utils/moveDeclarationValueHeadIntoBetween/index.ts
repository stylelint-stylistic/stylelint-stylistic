...
export function moveDeclarationValueHeadIntoBetween (syntax: Syntax, decl: Declaration, length: number): void {
	...
	// A raw holding nothing is read as no raw at all — `syntax.read` answers an empty one with `decl.value`, and that copy stands for text the move has just taken away — so where the head was the whole of the value the raw goes with it and the value is emptied to match
	if (tail === ``) {
