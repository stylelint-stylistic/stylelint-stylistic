...
/**
 * Reads the first statement of a stylesheet and asks whether its block is empty.
 * @param {string} cssString - The stylesheet.
 * @returns {boolean} What the util answers.
 */
function postcssCheck (cssString) {
	let root = parse(cssString)

	return hasEmptyBlock(/** @type {import('postcss').Rule} */ (root.first))
}
