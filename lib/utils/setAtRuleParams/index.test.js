...
describe(`setAtRuleParams`, () => {
	...
	it(`writes the copy the syntax prints`, () => {
		...
		expect(/** @type {import('../typeGuards/index.js').SyntaxRaw} */ (node.raws.params).scss).toBe(`screen // c\n  and (min-width: 2px)`)
	})
	...
	it(`keeps the raw beside it in step`, () => {
		...
		expect(/** @type {import('../typeGuards/index.js').SyntaxRaw} */ (node.raws.params).raw).toBe(`screen /* c*/\n  and (min-width: 2px)`)
	})
