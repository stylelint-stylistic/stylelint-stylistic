 ...
 *
 * Where `postcss-scss` keeps two copies of a set of parameters, the raw one it rewrote the `//`
 * comments in and the one spelled as the file spells it, the second is the one that is printed, so
 * the fix goes there. The raw is kept beside it in step, for the rules that come after: rewriting the
 * comments of the fixed parameters the way the syntax rewrites them is what fills it, so a rule
 * reading the pair is still handed the two copies of one text.
