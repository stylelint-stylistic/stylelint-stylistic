 ...
 *
 * PostCSS keeps a whitespace-only value in `decl.value`, so the file's closing break stands inside such a declaration. `declaration-colon-space-after` writes the run where it stands, taking that break, and no spelling satisfies both the option and a closed last line; the declaration is passed over, like one whose run has left it.
 *
