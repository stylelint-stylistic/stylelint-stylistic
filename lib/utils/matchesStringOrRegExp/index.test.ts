...
import { matchesStringOrRegExp } from "./index.ts"
...
describe(`matchesStringOrRegExp`, () => {
	...
	it(`comparing with a actual RegExp comparisonValue`, () => {
		expect(matchesStringOrRegExp(`.foo`, /.foo$/u)).toStrictEqual({
			match: `.foo`,
			pattern: /.foo$/u,
			substring: `.foo`,
		})
		expect(matchesStringOrRegExp(`bar .foo`, /.foo$/u)).toStrictEqual({
			match: `bar .foo`,
			pattern: /.foo$/u,
			substring: `.foo`,
		})
		expect(matchesStringOrRegExp(`bar .foo bar`, /.foo$/u)).toBe(false)
		expect(matchesStringOrRegExp(`foo`, /.foo$/u)).toBe(false)
		expect(matchesStringOrRegExp([`.foo`, `ebarz`], [/.foo$/u, /^bar/u])).toStrictEqual({
			match: `.foo`,
			pattern: /.foo$/u,
			substring: `.foo`,
		})
		expect(matchesStringOrRegExp([`foobar`], [/FOO/u])).toBe(false)
		expect(matchesStringOrRegExp([`FOOBAR`], [/FOO/u])).toStrictEqual({
			match: `FOOBAR`,
			pattern: /FOO/u,
			substring: `FOO`,
		})
