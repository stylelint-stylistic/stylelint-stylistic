 ...
 */
export function findInlineCommentSpans (text, spellsInlineComments = true) {
 ...
 * @param {Pick<import('postcss-value-parser').Node, 'sourceIndex' | 'sourceEndIndex'>} valueNode - The node about to be written back, of which only its span is read.
 * @param {InlineCommentSpan[]} spans - The spans {@link findInlineCommentSpans} found in the text the node was parsed from, which the node's positions count in.
 * @returns {InlineCommentSpan | undefined} The span the node overlaps, or nothing where the node carries no comment's text.
 */
export function findInlineCommentSpanTouching (valueNode, spans) {
