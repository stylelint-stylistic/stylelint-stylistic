...
/**
 * Gives a selector's inline comments their source spelling back, so that a fix written to
 * the raw suits the copy `postcss-scss` prints.
 *
 * A comment is found by the place it stands in among the block comments of the selector,
 * rather than by its text: the same text may stand in a comment of the other kind, or in a
 * quoted attribute value, and rewriting one of those would write code away.
...
export function restoreSelectorInlineComments (rawSelector, inlineComments) {
	...
	for (let inlineComment of inlineComments) {
		...
		// A selector the fix has left fewer comments in is one this cannot answer for, and the
		// rest of it goes back as it stands.
		if (!first || !last) break
