 ...
 *
 * `decl.toString()` prints `postcss-scss`'s copy of the value, each `//` comment rewritten as a block one, so the text is cut in front of the value and `syntax.read` supplies it. {@link declarationString} and {@link declarationColonSource} add what follows.
