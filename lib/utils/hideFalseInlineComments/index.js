 ...
 * less about the text than {@link findCommentSpans} does cannot take one for a comment.
 *
 ...
 * A slash a comment opens on is left alone, whichever of the two it is: the `/` of the `*\/` that
 * closes one comment and the `/` of the `/*` that opens the next spell a double slash between them,
 * and writing over the second of those would take the comment behind it away from the reader.
 * Inside the text of a comment the hyphen goes in like anywhere else: the reader is already inside
 * that comment when it reaches the text, and the copy is read for its code alone.
 * @param {string} text - The text to hide the false comment openings of.
 * @param {CommentSpan[]} [spans] - The spans the comments of the text occupy in it, both kinds of them, where they are already known. A caller that leaves one out leaves the slash it opens on unguarded.
 * @returns {string} The text, with the second slash of every false comment opening written as a hyphen.
 */
export function hideFalseInlineComments (text, spans = findCommentSpans(text)) {
