...
let html = /** @type {{ parse: import('postcss').Parser }} */ (postcssHtml)
 ...
 * @param {{ parse: import('postcss').Parser }} syntax - The syntax to parse with.
...
function parentsOf (syntax, code) {
	...
	syntax.parse(code).walkDecls((decl) => {
		verdicts.push(isInlineStyleAttribute(/** @type {import('postcss').Container} */ (decl.parent)))
	})
