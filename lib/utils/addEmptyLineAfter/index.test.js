 ...
 * @param {number} [index] - Which statement of it, counted from the top.
 ...
 */
function run (css, index = 0, rules = {}) {
	let root = parse(css)

	addEmptyLineAfter(/** @type {import('postcss').Rule} */ (root.nodes[index]), /** @type {import('stylelint').PostcssResult} */ (/** @type {unknown} */ ({ stylelint: { config: { rules } } })))
