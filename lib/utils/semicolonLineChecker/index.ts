 ...
 *
 * Such a line is an empty statement of its container, so it is asked for the level a statement with no block stands at there. Nobody else measures it (1790234713): the node's check reads the last line of `raws.before`, the brace's the last line of the run in front of the brace, and the parser files the line in one of four raws: the middle lines of a node's `raws.before`, the middle lines of the run in front of a closing brace, the last line of a rule's `raws.ownSemicolon` where a break or the end of the file closes it, and the lines of a root's `raws.after`, its last one only where the root is the file. The line of a node or a brace behind the semicolon is theirs ([#516](https://github.com/stylelint-stylistic/stylelint-stylistic/issues/516), 1789424028), and the first line of a raw opens behind the text in front of it, unless the raw opens the file or the host code in front of it ends in a break. The warning stands on the semicolon, counted from a point of the raw whose place in the file is known, and the fix writes that one line. The check runs behind the node's and the brace's own fixes, which write every line of their raw at their level.
